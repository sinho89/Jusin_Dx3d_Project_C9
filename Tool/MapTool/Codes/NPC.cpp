...
void CNPC::SettingWork( void )
{
	...
	else if(m_eMode == MODE_ANGLE)
	{
		...
		//{
		//	m_pTransCom->Rotation(Engine::ROT_X, D3DXToRadian(90.f * m_lMouseMove * 0f.01f));
		//}
...
void CNPC::Complete( void )
{
	m_bComplete = true;
}

OBJECT_MODE CNPC::GetMode( void )
{
	return m_eMode;
}

void CNPC::SetMode( OBJECT_MODE eMode )
{
	m_eMode = eMode;
}
...
_bool CNPC::GetChecked( void )
{
	return m_bCheck;
}

void CNPC::CheckTrue( void )
{
	m_bCheck = true;
}

void CNPC::CheckFalse( void )
{
	m_bCheck = false;
}
