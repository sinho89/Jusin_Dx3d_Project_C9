...
HRESULT CBabegazi_Warrior_Ani::Ready_State(Engine::CDynamicMesh* pMesh, map<_uint, list<Engine::ANIMATORINFO*>>* pmapNextMotion, _uint iAniIndex)
{
	if(pMesh == NULL)
		return E_FAIL;
	m_pMesh = pMesh;
...
CBabegazi_Warrior_Ani* CBabegazi_Warrior_Ani::Create(Engine::CDynamicMesh* pMesh, map<_uint, list<Engine::ANIMATORINFO*>>* pmapNextMotion, _uint iAniIndex)
{
	...
	if(FAILED(pInstance->Ready_State(pMesh, pmapNextMotion, iAniIndex)))
	{
