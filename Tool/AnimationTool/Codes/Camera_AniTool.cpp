...
CCamera_AniTool::CCamera_AniTool(LPDIRECT3DDEVICE9 pGraphicDev, const wstring& wstrName)
...
CCamera_AniTool::~CCamera_AniTool(void)
{

}

void CCamera_AniTool::Update_Object(const _float& fTimeDelta)
{
	KeyCheck(fTimeDelta);
	MouseMove();
...
HRESULT CCamera_AniTool::Ready_Camera(const _vec3* pEye, const _vec3* pAt, 
...
void CCamera_AniTool::KeyCheck(const _float& fTimeDelta)
{
	_matrix			matWorld;
...
void CCamera_AniTool::MouseMove(void)
{
	_long	dwMouseMove = 0;
	_matrix			matWorld;
	D3DXMatrixInverse(&matWorld, NULL, &m_matView);
...
CCamera_AniTool* CCamera_AniTool::Create(LPDIRECT3DDEVICE9 pGraphicDev, const wstring& wstrName, 
...
{
	CCamera_AniTool*			pInstance = new CCamera_AniTool(pGraphicDev, wstrName);
...
void CCamera_AniTool::Free(void)
{
	Engine::CCamera::Free();
}