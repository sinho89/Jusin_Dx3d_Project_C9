...
BOOL CMainFrame::OnCreateClient(LPCREATESTRUCT lpcs, CCreateContext* pContext)
{
	...
	m_MainSplitter.CreateView(0, 1, RUNTIME_CLASS(CAnimationToolView), CSize(WINSIZEX, WINSIZEY), pContext);
	m_MainSplitter.CreateView(0, 0, RUNTIME_CLASS(CMyForm), CSize(300, 700), pContext);

	//1.부모 윈도우 주소...
	//2.열 개수
	//3.행 개수
	//4.창 스타일
	//5.창틀을 위한 윈도우가 들어가면 됨...
	//0,0, 번째에 해당하는 mainsplitter 의 자식 윈도우를 가져 온 것임...
	m_SecondSplitter.CreateStatic(&m_MainSplitter, 2, 1, WS_CHILD | WS_VISIBLE,
		m_MainSplitter.IdFromRowCol(0, 0));

	//RecalcLayout();

	//행, 열을 지정하면 CWnd* 반환한다....
	m_pMainView = (CAnimationToolView*)m_MainSplitter.GetPane(0, 1);
