...
void CParticle::Update_Object(const _float& fTimeDelta)
{
	...
	case Engine::MOVE_CYCLONE:
		{
			CycloneMoveUpdate();	 //운동설정
			CycloneUpdate_RePart(); //움직임 적용
		}
		break;
	}
...
HRESULT CParticle::SetUp_ConstantTable(LPD3DXEFFECT pEffect)
{
	...
	float m_fFar = ((CEffectCamera*)m_pCamObject)->GetCamFar();
...
void CParticle::SetPart(_int nIdx)
{
	...
	//초기 회전
	pPrt->CrnR.x	= 20.f + rand()%360;
	pPrt->CrnR.y	= 20.f + rand()%360;
	pPrt->CrnR.z	= 20.f + rand()%360;

	//회전 속도
	pPrt->CrnRv.x	=(rand()%91-25.5f) *1.2f;
	pPrt->CrnRv.y	=(rand()%91-25.5f) *1.2f;
	pPrt->CrnRv.z	=(rand()%91-25.5f) *1.2f;

	pPrt->CrnRv	*=0.1f;


	if ((m_pParticleInfo.m_vScale.x == 0.0f && m_pParticleInfo.m_vScale.y == 0.0f && m_pParticleInfo.m_vScale.z == 0.0f ) &&
		(m_vLoadScale.x != 0.0f && m_vLoadScale.y != 0.0f && m_vLoadScale.z != 0.0f))
	{
		m_pParticleInfo.m_vScale = m_vLoadScale;
	}
	...
	_vec4 Color = _vec4((rand()%255)/255.0f, (rand()%255)/255.0f,  (rand()%255)/255, 1.f);

	//pPrt->dColor =	m_pParticleInfo.m_dColor;
	pPrt->dColor = (D3DXCOLOR)Color;
...
void CParticle::MoveUpdate(void)
{
	...
	_float fTime = m_pParticleInfo.m_fTimeAvg * 0.5f;
...
void CParticle::LoadParticleInfo(Engine::PARTICLEINFO* info)
{
	...
	m_vLoadScale = m_pParticleInfo.m_vScale;
...
void CParticle::SetPath(int path)
{
...
//나선효과
void CParticle::CycloneMoveUpdate(void)
{
	//1.운동 갱신
	...
	for(_int i = 0; i < m_pParticleInfo.m_PrtN; ++i)
	{
		...
		//float angle = D3DXToRadian(stcount);


		//pPrt->CrnP.x += 5 * cosf(angle) * fTime;
		//pPrt->CrnP.z += 5 * sinf(angle) * fTime;
		//pPrt->CrnP.y += 0.2f;
...
void CParticle::CycloneUpdate_RePart(void)
{
	D3DXMATRIX  matX;
	D3DXMATRIX	matY;
	D3DXMATRIX	matZ;
	...
	for(_int i = 0; i < m_pParticleInfo.m_PrtN; ++i)
	{
		...
		if(m_pParticleInfo.m_bBillBoard)
		{
			m_matWorld = matS * matBill * TransmatWorld *  matT;
		}
		else
		{
			m_matWorld = matS * matX * matY * matZ * matT;
...
void CParticle::CycloneSetPart(_int nIdx)
{
	Engine::TPART* pPrt = &m_PrtD[nIdx];
	...
	//pPrt->IntP.x	=  m_pParticleInfo.m_vParticleArrange.x * cosf(fPhi);

	pPrt->IntP.x	=  m_fCycloneRadius * cosf(fPhi);

	//pPrt->IntP.y	=  (nIdx)%(int)m_pParticleInfo.m_vParticleArrange.y;

	pPrt->IntP.y	=  (nIdx) * m_pParticleInfo.m_fCycloneHeightInterval;

	//pPrt->IntP.z	=  m_pParticleInfo.m_vParticleArrange.z * sinf(fPhi);
...
void CParticle::SetCycloneInterval(_float result, _int select)
{
	switch(select)
	{
	case 0: //RadiusInterval
		m_pParticleInfo.m_fCycloneRadiusInterval = result;
		break;

	case 1: //AngleInterval
		m_pParticleInfo.m_fCycloneAngleInterval = result;
		break;

	case 2: //HightInterval
		m_pParticleInfo.m_fCycloneHeightInterval = result;
		break;
	}

}

void CParticle::InitCycloneReset(void)
{
	m_fCycloneRadius = 0.0f;
	m_fCycloneAngle = 0.0f;
}

void CParticle::ParentUpdateMatrix(void)
{
	if(m_pParticleInfo.m_eMoveMent == Engine::MOVE_CYCLONE)
	{
		m_pTransCom->Rotation(Engine::ROT_Y, m_pParticleInfo.m_fTimeAvg * 0.1f);
	}

}

HRESULT CParticle::Start_ForScene(void)
{
	if (m_bOnceStart)
		return S_OK;
	...
	Engine::CGameObject* pGameObject = Engine::FindWithTag(L"Camera");

	if (pGameObject == NULL)
		return S_OK;


	m_pCamObject = pGameObject;
