...
CButton_SSAO::CButton_SSAO(LPDIRECT3DDEVICE9 pGraphicDev, const wstring& wstrName, const Engine::OBJECTSTATE& eState /*= Engine::STATE_DEFAULT*/, const _bool bIsActive /*= true*/)
...
CButton_SSAO::~CButton_SSAO(void)
{

}

HRESULT CButton_SSAO::Ready_Object(void)
{
	...
	if (m_bType)
	{
		m_fX = m_pParent->GetInfo().x + 50;
		m_fY = m_pParent->GetInfo().y - 125.f;
	...
	{
		m_fX = m_pParent->GetInfo().x + 120.f;
		m_fY = m_pParent->GetInfo().y - 125.f;
...
HRESULT CButton_SSAO::Start_ForScene(void)
{
...
void CButton_SSAO::Update_Object(const _float& fTimeDelta)
{
	...
	if (m_bRender)
	{
		...
		{
			m_fX = m_pParent->GetInfo().x + 50;
			m_fY = m_pParent->GetInfo().y - 120.f;
		}
		else
		{
			m_fX = m_pParent->GetInfo().x + 120.f;
	...
	{
		Engine::SetSSAO(TRUE);
		g_ShaderOption_Info.bSSAO = TRUE;
	}
...
void CButton_SSAO::Render_Object(void)
{
...
HRESULT CButton_SSAO::Add_Component(void)
{
...
void CButton_SSAO::PtInRect_Collision(const _float& fTimeDelta)
{
	POINT		ptMouse;
	...
	if (m_bCollision)
	{
		if (Engine::Get_DIMouseState(Engine::CInput_Device::DIM_LB) & 0x80)
		{
			...
			if (m_bType)
			{
				CUI* pUI = ((CUI*)Engine::Find(L"System_SSAO_Off_Button"));
				pUI->SetPick(false);
				::Safe_Release(pUI);
			}
			else
			{
				CUI* pUI = ((CUI*)Engine::Find(L"System_SSAO_On_Button"));
				pUI->SetPick(false);
				::Safe_Release(pUI);
			}
			m_bPick = true;
...
void CButton_SSAO::SetParent(CUI* pParent)
{
	m_pParent = pParent;
}

CButton_SSAO* CButton_SSAO::Create(LPDIRECT3DDEVICE9 pGraphicDev, const wstring& wstrName, const Engine::OBJECTSTATE& eState /*= Engine::STATE_DEFAULT*/, const _bool bIsActive /*= true*/)
{
	CButton_SSAO*		pInstance = new CButton_SSAO(pGraphicDev, wstrName, eState, bIsActive);
...
CButton_SSAO* CButton_SSAO::Create(LPDIRECT3DDEVICE9 pGraphicDev, const wstring& wstrName, CUI* pParent, _bool bType, Engine::CScene* pScene, const Engine::OBJECTSTATE& eState /*= Engine::STATE_DEFAULT*/, const _bool bIsActive /*= true*/)
{
	CButton_SSAO*		pInstance = new CButton_SSAO(pGraphicDev, wstrName, eState, bIsActive);
