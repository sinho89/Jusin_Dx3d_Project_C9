...
CFrameProjector::CFrameProjector(LPDIRECT3DDEVICE9 pGraphicDev, const wstring & wstrName)
...
, m_dTime(0)
{
	...
	m_iBlurPass = 2;
	m_iBlurCnt = 2;
	m_fBlurPower = 2.f;        	 // 높을수록 밝게
	m_fBlurValue = 100.0f;           //낮을수록 많이 흔들린다.
	m_eBlurType = Engine::BLUR_CROSS;
}

CFrameProjector::~CFrameProjector(void)
{
}

void CFrameProjector::Update_Object(const _float & fTimeDelta)
{
	if (!m_bAni)
		return;
...
void CFrameProjector::Render_Object(void)
{
	if (!m_bAni)
		return;
...
void CFrameProjector::Render_Object(LPD3DXEFFECT pEffect, const _uint iPassIdx)
{
	pEffect->AddRef();
...
HRESULT CFrameProjector::Ready_Object(void)
{
...
CFrameProjector * CFrameProjector::Create(LPDIRECT3DDEVICE9 pGraphicDev, const wstring & wstrName)
{
	CFrameProjector*		pInstance = new CFrameProjector(pGraphicDev, wstrName);

	if (FAILED(pInstance->Ready_Object()))
	{
		MSG_BOX("FrameProjector Created Failed");
...
void CFrameProjector::Free(void)
{
	CEffect::Free();
	Engine::CEffect::Destroy();
}

HRESULT CFrameProjector::Add_Component(void)
{
...
HRESULT CFrameProjector::SetUp_ConstantTable(LPD3DXEFFECT pEffect)
{
	pEffect->AddRef();
...
void CFrameProjector::SetPart(_int nIdx)
{
...
void CFrameProjector::MoveUpdate(const _float & fTimeDelta)
{
	//1.운동 갱신
	if (m_dTime >= 0.03f) //생성 함수
	{
		m_dTime = 0;
		...
		for (int i = 0; i< m_pParticleInfo.m_PrtN; ++i)
		{
			...
			if (pPrt->bLive == true) //현재 진행중인 파티클인 경우
				continue;

			//죽어있는 경우의 파티클인 경우
			this->SetPart(i);
			pPrt->bLive = true; //죽어있는 파티클 bLive를 살린다.
			pPrt->CrnP = m_vTargetPos;

			pPrt->CrnV = m_vTargetDir;
...
void CFrameProjector::Update_RePart(void)
{
	D3DXMATRIX  matX;
	D3DXMATRIX	matY;
	D3DXMATRIX	matZ;
...
void CFrameProjector::SetAni(_bool bAni)
{
	m_bAni = bAni;
...
_bool CFrameProjector::GetAni(void)
{
	return m_bAni;
}

CFrameProjector * CFrameProjector::LoadCreate(LPDIRECT3DDEVICE9 pGraphicDev, const wstring & wstrName)
{
	CFrameProjector*		pInstance = new CFrameProjector(pGraphicDev, wstrName);
...
HRESULT CFrameProjector::LoadReady_Object(void)
{
	if (FAILED(Add_Component()))
		return E_FAIL;
...
void CFrameProjector::SetTargetDir(_vec3 m_pDir)
{
	m_vTargetDir = m_pDir;
}

void CFrameProjector::SetTargetPos(_vec3 m_vPos)
{
	m_vTargetPos = m_vPos;
}
