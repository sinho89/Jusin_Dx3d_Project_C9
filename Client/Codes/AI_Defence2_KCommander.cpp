...
CAI_Defence2_KCommander::CAI_Defence2_KCommander(void)
	:CAI_Knole_Commander()
	, m_bAttackCol(false)
	, m_bAttackTime(false)
	, m_fAttackTime(0.f)
...
HRESULT CAI_Defence2_KCommander::Ready_State(Engine::CTransform * pTransform, Engine::CAnimator * pAnimator,
...
void CAI_Defence2_KCommander::Start_State(void)
{
...
void CAI_Defence2_KCommander::Update_State(const _float & fTimeDelta)
{
	...
	if (m_pAnimator->Get_AnimationIndex() == DEFENCE_START)
	{
		m_pAnimator->Set_AnimationSet(DEFENCE_ING);
	}
//////////////////어택시 위치값 이동..
	if (m_pAnimator->Get_AnimationIndex() == DEFENCE_ING)
	{
		...
		if (m_pAnimator->IsPlaying_ThisSection((*m_pvecAniInfo)[DEFENCE_ING]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[DEFENCE_ING]->tMotionInfo.dEndTime * 0.1,
			(*m_pvecAniInfo)[DEFENCE_ING]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[DEFENCE_ING]->tMotionInfo.dEndTime * 0.9))
		{
			if (!m_bAttackCol)
			{
				m_bAttackCol = true;
		...
		if (m_bAttackTime)
		{
			m_pAnimator->Set_AnimationSet(DEFENCE_END);
		}
	}
	///////////////////////////////////////////
	if (m_pAnimator->Get_AnimationIndex() == DEFENCE_END)
	{
...
void CAI_Defence2_KCommander::End_State(void)
{
	ZeroMemory(m_bCol, sizeof(bool) * Engine::COL_INFO_END);
	m_bAttackCol = false;
}

void CAI_Defence2_KCommander::Collsion_Work(void)
{
	if (*m_pDamaged)
	{
...
CAI_Defence2_KCommander * CAI_Defence2_KCommander::Create(Engine::CTransform * pTransform, Engine::CAnimator * pAnimator,
	Engine::CNavigationMesh* pNaviMesh, Engine::CCalculator* pCalculator, Engine::CRigidbody* pRigidbody
	, Engine::CCollider* pCollider, Engine::COLINFO* pColInfo, _bool* pDamaged, _bool* pWeapon
	, Engine::CAI* pAI)
{
	CAI_Defence2_KCommander* pInstance = new CAI_Defence2_KCommander;
	...
	{
		MSG_BOX("CAI_Defence2_KCommander Created Failed");
...
void CAI_Defence2_KCommander::Free(void)
{
