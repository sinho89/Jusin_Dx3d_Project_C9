...
CAI_Down_BAxe::CAI_Down_BAxe(void)
	:CAI_Babegazi_Axe()
	, m_fDownTime(0.f)
	, m_bStand(false)
	, m_iDirection(0)
...
HRESULT CAI_Down_BAxe::Ready_State(Engine::CTransform* pTransform, Engine::CAnimator* pAnimator,
...
void CAI_Down_BAxe::Start_State(void)
{
...
void CAI_Down_BAxe::Update_State(const _float & fTimeDelta)
{
...
void CAI_Down_BAxe::End_State(void)
{
	ZeroMemory(m_bCol, sizeof(bool) * Engine::COL_INFO_END);
}

void CAI_Down_BAxe::Collsion_Work(void)
{
	if (*m_pDamaged)
	{
...
CAI_Down_BAxe * CAI_Down_BAxe::Create(Engine::CTransform* pTransform, Engine::CAnimator* pAnimator,
	Engine::CNavigationMesh* pNaviMesh, Engine::CCalculator* pCalculator,
	Engine::CRigidbody* pRigidbody, Engine::CCollider* pCollider,
	Engine::COLINFO* pColInfo, _bool* pDamaged,
	Engine::CAI* pAI)
{
	CAI_Down_BAxe* pInstance = new CAI_Down_BAxe;

	if (FAILED(pInstance->Ready_State(pTransform, pAnimator, pNaviMesh, pCalculator, pRigidbody
	, pCollider, pColInfo, pDamaged, pAI)))
	{
		MSG_BOX("CAI_Down_BAxe Created Failed");
