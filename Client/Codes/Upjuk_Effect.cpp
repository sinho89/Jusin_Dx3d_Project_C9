...
CUpjuk_Effect::CUpjuk_Effect(LPDIRECT3DDEVICE9 pGraphicDev, const wstring & wstrName)
	: CEffect(pGraphicDev, wstrName)
{
}

CUpjuk_Effect::~CUpjuk_Effect(void)
{
}

void CUpjuk_Effect::Update_Object(const _float & fTimeDelta)
{
...
void CUpjuk_Effect::Render_Object(void)
{
...
HRESULT CUpjuk_Effect::Ready_Object(void)
{
	...
	m_iMaxFrame = 16;//스프라이트의 총개수
	m_iRow = 4;  //스프라이트 가로 개수
	m_iColumn = 4; //세로 개수
	m_fSpriteSpeed = 16.f;//스프라이트 속도
...
CUpjuk_Effect * CUpjuk_Effect::Create(LPDIRECT3DDEVICE9 pGraphicDev, const wstring & wstrName)
{
	CUpjuk_Effect*		pInstance = new CUpjuk_Effect(pGraphicDev, wstrName);
...
CUpjuk_Effect* CUpjuk_Effect::Create(LPDIRECT3DDEVICE9 pGraphicDev, const wstring& wstrName, CUI* pParent)
{
	CUpjuk_Effect*		pInstance = new CUpjuk_Effect(pGraphicDev, wstrName);
...
HRESULT CUpjuk_Effect::Add_Component(void)
{
...
HRESULT CUpjuk_Effect::SetUp_ConstantTable(LPD3DXEFFECT pEffect)
{
	pEffect->AddRef();
...
void CUpjuk_Effect::Update_RePart(void)
{
	D3DXMATRIX  matX;
	D3DXMATRIX	matY;
	D3DXMATRIX	matZ;
...
void CUpjuk_Effect::SpriteUpdate(const _float & fTimeDelta)
{
	//스프라이트 업데이트를 돌려준다.
	Engine::TPART* pPrt = m_PrtD;
	...
	if (pPrt->PrsImg >= m_iMaxFrame)
	{
		...
		if (m_fAlpha <= 0.5f)
		{
			...
			{
				Engine::CGameObject* pGameObject = CUpjuk_Effect2::Create(m_pGraphicDev, L"Upjuk_Effect2", m_pParent);
...
void CUpjuk_Effect::SetPart(void)
{
	Engine::TPART* pPrt = m_PrtD;
