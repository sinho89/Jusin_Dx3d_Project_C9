...
HRESULT CMiniPlayer::Start_ForScene(void)
{
	...
	if (Engine::GetSceneID() == Engine::SCENE_TOWN || Engine::GetSceneID() == Engine::SCENE_STAGE_FIRE || Engine::GetSceneID() == Engine::SCENE_STAGE_ICE)
		m_bIsActive = true;
...
void CMiniPlayer::Update_Object(const _float& fTimeDelta)
{
...
void CMiniPlayer::Render_Object(void)
{
...
HRESULT CMiniPlayer::Add_Component(void)
{
	...
	m_pTextureCom = (Engine::CTexture*)Engine::Clone_Resource(RESOURCE_STAGESTATIC, L"Texture_MiniMap_Player");
...
CMiniPlayer* CMiniPlayer::Create(LPDIRECT3DDEVICE9 pGraphicDev, 
								 ...
								 const _bool bIsActive /*= true*/)
{
	CMiniPlayer* pMiniPlayer = new CMiniPlayer(pGraphicDev, wstrName);

	if(FAILED(pMiniPlayer->Ready_Object()))
	{
		MSG_BOX("CMiniPlayer Created Failed");
		::Safe_Release(pMiniPlayer);
	}
