...
CPenaltyTrail::CPenaltyTrail(LPDIRECT3DDEVICE9 pGraphicDev, const wstring & wstrName)
...
CPenaltyTrail::~CPenaltyTrail(void)
{
}

void CPenaltyTrail::Update_Object(const _float & fTimeDelta)
{
	if (!m_bAni)
		return;
...
void CPenaltyTrail::Render_Object(void)
{
	if (!m_bAni)
		return;
...
void CPenaltyTrail::Render_Object(LPD3DXEFFECT pEffect, const _uint iPassIdx)
{
	if (!m_bAni)
		return;
...
HRESULT CPenaltyTrail::Ready_Obejct(void)
{
	m_iCntX = 30;
	m_iCntZ = 2;
	m_iInterval = 1;

	if (L"Penalty_RightHand" == m_wstrName)
	{
		if (FAILED(Engine::Ready_Buffers(m_pGraphicDev, RESOURCE_STAGESTATIC, L"Bardiel_Penalty_RightHand_Trail", Engine::BUFFER_TRAILTEX, m_iCntX, m_iCntZ, m_iInterval)))
		{
			MSG_BOX("Bardiel_Penalty_RightHand_Trail Load Failed");
			return E_FAIL;
		}
	}
	else if (L"Penalty_LeftHand" == m_wstrName)
	{
		if (FAILED(Engine::Ready_Buffers(m_pGraphicDev, RESOURCE_STAGESTATIC, L"Bardiel_Penalty_LeftHand_Trail", Engine::BUFFER_TRAILTEX, m_iCntX, m_iCntZ, m_iInterval)))
		{
			MSG_BOX("Bardiel_Penalty_LeftHand_Trail Load Failed");
			return E_FAIL;
		}
	}	
...
CPenaltyTrail * CPenaltyTrail::Create(LPDIRECT3DDEVICE9 pGraphicDev, const wstring & wstrName)
{
	CPenaltyTrail*		pInstance = new CPenaltyTrail(pGraphicDev, wstrName);

	if (FAILED(pInstance->Ready_Obejct()))
	{
		MSG_BOX("PenaltyTrail Created Failed");
...
void CPenaltyTrail::Free(void)
{
	Engine::Safe_Delete_Array(m_pTrailVtx);
	CEffect::Free();
}

HRESULT CPenaltyTrail::Add_Component(void)
{
	...
	if (L"Penalty_RightHand" == m_wstrName)
	{
		m_pBufferCom = (Engine::CTrail_Texture*)Engine::Clone_Resource(RESOURCE_STAGESTATIC, L"Bardiel_Penalty_RightHand_Trail");
	...
	{
		m_pBufferCom = (Engine::CTrail_Texture*)Engine::Clone_Resource(RESOURCE_STAGESTATIC, L"Bardiel_Penalty_LeftHand_Trail");
		pComponent = (Engine::CComponent*)m_pBufferCom;
		if (NULL == pComponent)
			return E_FAIL;
		m_mapComponent[Engine::COMID_STATIC].insert(MAPCOMPONENT::value_type(L"Com_Buffer", pComponent));
...
HRESULT CPenaltyTrail::SetUp_ConstantTable(LPD3DXEFFECT pEffect)
{
	pEffect->AddRef();
...
void CPenaltyTrail::SettingTrail(void)
{
	vStart = _vec3(0.0f, 0.0f, 0.0f);
	vEnd = _vec3(0.0f, 0.0f, 0.0f);
...
void CPenaltyTrail::SetAni(_bool bAni)
{
	m_bAni = bAni;
}

_bool CPenaltyTrail::GetAni(void)
{
	return m_bAni;
}

void CPenaltyTrail::SetTargetMatrix(const _matrix * pTarget_Start_Mat, const _matrix * pTarget_End_Mat)
{
