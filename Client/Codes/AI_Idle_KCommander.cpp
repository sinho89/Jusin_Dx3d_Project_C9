...
CAI_Idle_KCommander::CAI_Idle_KCommander(void)
	:CAI_Knole_Commander()
	, m_bOne(false)
	, m_bIdleTime(false)
	, m_fIdleTime(0.f)
...
HRESULT CAI_Idle_KCommander::Ready_State(Engine::CTransform * pTransform, Engine::CAnimator * pAnimator,
...
void CAI_Idle_KCommander::Start_State(void)
{
...
void CAI_Idle_KCommander::Update_State(const _float & fTimeDelta)
{
	...
	if (!m_bDamaged)
	{
		if (m_pAnimator->Get_AnimationIndex() == MODE_2_BATTLE)
		{
			if (m_pAnimator->IsPlaying_ThisSection((*m_pvecAniInfo)[MODE_2_BATTLE]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[MODE_2_BATTLE]->tMotionInfo.dEndTime * 0.4,
				(*m_pvecAniInfo)[MODE_2_BATTLE]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[MODE_2_BATTLE]->tMotionInfo.dEndTime * 0.6))
			{
				*m_pWeapon = true;
			}
	...
	if (!m_bDamaged)
	{
		...
		if (m_bIdleTime)
		{
			...
			{
				if (m_pAnimator->Get_AnimationIndex() == BATTLE_IDLE_1)
				{
					*m_pWeapon = false;
...
void CAI_Idle_KCommander::End_State(void)
{
	ZeroMemory(m_bCol, sizeof(bool) * Engine::COL_INFO_END);
}

void CAI_Idle_KCommander::Collsion_Work(void)
{
	if (*m_pDamaged)
	{
...
CAI_Idle_KCommander * CAI_Idle_KCommander::Create(Engine::CTransform * pTransform, Engine::CAnimator * pAnimator,
	Engine::CNavigationMesh* pNaviMesh, Engine::CCalculator* pCalculator, Engine::CRigidbody* pRigidbody
	, Engine::CCollider* pCollider, Engine::COLINFO* pColInfo, _bool* pDamaged, _bool* pWeapon
	, Engine::CAI* pAI)
{
	CAI_Idle_KCommander* pInstance = new CAI_Idle_KCommander;
...
void CAI_Idle_KCommander::Free(void)
{
