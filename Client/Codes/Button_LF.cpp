...
CButton_LF::CButton_LF(LPDIRECT3DDEVICE9 pGraphicDev, const wstring& wstrName, const Engine::OBJECTSTATE& eState /*= Engine::STATE_DEFAULT*/, const _bool bIsActive /*= true*/)
...
CButton_LF::~CButton_LF(void)
{

}

HRESULT CButton_LF::Ready_Object(void)
{
	...
	if (m_bType)
	{
		...
		m_fY = m_pParent->GetInfo().y + 130.f;
...
HRESULT CButton_LF::Start_ForScene(void)
{
...
void CButton_LF::Update_Object(const _float& fTimeDelta)
{
	...
	if (m_bRender)
	{
		...
		if (m_bType)
		{
			m_fX = m_pParent->GetInfo().x + 110;
			m_fY = m_pParent->GetInfo().y + 200.f;
		}
		else
		{
	...
	if (m_bType && m_bPick)
	{
		Engine::SetLimLight(TRUE);
		g_ShaderOption_Info.bLimLight = TRUE;
	}
...
void CButton_LF::Render_Object(void)
{
...
HRESULT CButton_LF::Add_Component(void)
{
...
void CButton_LF::PtInRect_Collision(const _float& fTimeDelta)
{
	POINT		ptMouse;
	...
	if (m_bCollision)
	{
		if (Engine::Get_DIMouseState(Engine::CInput_Device::DIM_LB) & 0x80)
		{
			...
			if (m_bType)
			{
				CUI* pUI = ((CUI*)Engine::Find(L"System_LF_Off_Button"));
				pUI->SetPick(false);
				::Safe_Release(pUI);
			}
			else
			{
				CUI* pUI = ((CUI*)Engine::Find(L"System_LF_On_Button"));
				pUI->SetPick(false);
				::Safe_Release(pUI);
			}
			m_bPick = true;
...
void CButton_LF::SetParent(CUI* pParent)
{
	m_pParent = pParent;
}

CButton_LF* CButton_LF::Create(LPDIRECT3DDEVICE9 pGraphicDev, const wstring& wstrName, const Engine::OBJECTSTATE& eState /*= Engine::STATE_DEFAULT*/, const _bool bIsActive /*= true*/)
{
	CButton_LF*		pInstance = new CButton_LF(pGraphicDev, wstrName, eState, bIsActive);
...
CButton_LF* CButton_LF::Create(LPDIRECT3DDEVICE9 pGraphicDev, const wstring& wstrName, CUI* pParent, _bool bType, Engine::CScene* pScene, const Engine::OBJECTSTATE& eState /*= Engine::STATE_DEFAULT*/, const _bool bIsActive /*= true*/)
{
	CButton_LF*		pInstance = new CButton_LF(pGraphicDev, wstrName, eState, bIsActive);
