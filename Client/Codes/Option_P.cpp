...
COption_P::COption_P(LPDIRECT3DDEVICE9 pGraphicDev, const wstring& wstrName, const Engine::OBJECTSTATE& eState /*= Engine::STATE_DEFAULT*/, const _bool bIsActive /*= true*/)
...
COption_P::~COption_P(void)
{

}

HRESULT COption_P::Ready_Object(void)
{
	if(FAILED(Add_Component()))
		return E_FAIL;

	m_fX = (WINSIZEX >> 1) - 180.f;
	m_fY = (WINSIZEY >> 1);
...
HRESULT COption_P::Start_ForScene(void)
{
...
void COption_P::Update_Object(const _float& fTimeDelta)
{
...
void COption_P::Render_Object(void)
{
	...
	if(m_bClick || m_bCollision)
	{
		Engine::Render_Font(L"Font_Main_Data", L"(P) Á¤º¸ ", &_vec2(m_fX - 30.f, m_fY - 45.f), D3DXCOLOR(1.f, 1.f, 1.f, m_fAlpha));
	}
}


HRESULT COption_P::Add_Component(void)
{
	...
	m_pNCTextureCom = (Engine::CTexture*)Engine::Clone_Resource(RESOURCE_STAGESTATIC, L"Option_P");
	...
	m_pCTextureCom = (Engine::CTexture*)Engine::Clone_Resource(RESOURCE_STAGESTATIC, L"Option_P_C");
	...
	m_pClickTextureCom = (Engine::CTexture*)Engine::Clone_Resource(RESOURCE_STAGESTATIC, L"Option_P_Click");
...
void COption_P::Alpha_Calculation(const _float& fTimeDelta)
{
	if(!m_bRender)
	{
...
void COption_P::PtInRect_Collision(const _float& fTimeDelta)
{
	POINT		ptMouse;
	...
	if(m_bCollision)
	{
		...
		else
		{
			if(m_bClick)
			{
				...
				CUI*	pUI	= ((CUI*)Engine::Find(L"Window_Info"));
				if(pUI->GetRender())
					pUI->SetRender(false);
				else
					pUI->SetRender(true);
...
COption_P* COption_P::Create(LPDIRECT3DDEVICE9 pGraphicDev, const wstring& wstrName, const Engine::OBJECTSTATE& eState /*= Engine::STATE_DEFAULT*/, const _bool bIsActive /*= true*/)
{
	COption_P*		pInstance = new COption_P(pGraphicDev, wstrName, eState, bIsActive);
