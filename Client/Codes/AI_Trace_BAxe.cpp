...
CAI_Trace_BAxe::CAI_Trace_BAxe(void)
	:CAI_Babegazi_Axe()
	, m_fCompareTime(0.f)
	, m_bCompareDis(false)
	, m_dwIdx(0)
...
HRESULT CAI_Trace_BAxe::Ready_State(Engine::CTransform * pTransform, Engine::CAnimator * pAnimator,
...
void CAI_Trace_BAxe::Start_State(void)
{
...
void CAI_Trace_BAxe::Update_State(const _float & fTimeDelta)
{
...
void CAI_Trace_BAxe::End_State(void)
{
	ZeroMemory(m_bCol, sizeof(bool) * Engine::COL_INFO_END);
}

//void CAI_Trace_BAxe::CubeRayCheck(void)
//{
...
void CAI_Trace_BAxe::TraceMove(const _float& fTimeDelta)
{
	if (m_bRayMove)
	{
		m_pAnimator->Set_AnimationSet(MOVE_RUN);
...
_vec3 CAI_Trace_BAxe::AStarMove(_vec3* pPos, const _float &fTimeDelta)
{
	if (m_BestList.empty())
	{
		return *pPos;
...
void CAI_Trace_BAxe::Collsion_Work(void)
{
...
CAI_Trace_BAxe * CAI_Trace_BAxe::Create(Engine::CTransform * pTransform, Engine::CAnimator * pAnimator,
	Engine::CNavigationMesh* pNaviMesh, Engine::CCalculator* pCalculator, Engine::CRigidbody* pRigidbody
	, Engine::CCollider* pCollider, Engine::COLINFO* pColInfo, _bool* pDamaged
	, Engine::CAI* pAI)
{
	CAI_Trace_BAxe* pInstance = new CAI_Trace_BAxe;

	if (FAILED(pInstance->Ready_State(pTransform, pAnimator, pNaviMesh, pCalculator, pRigidbody, pCollider
		, pColInfo, pDamaged, pAI)))
	{
		MSG_BOX("CAI_Trace_BAxe Created Failed");
