...
COption_Skill::COption_Skill(LPDIRECT3DDEVICE9 pGraphicDev, const wstring& wstrName, const Engine::OBJECTSTATE& eState /*= Engine::STATE_DEFAULT*/, const _bool bIsActive /*= true*/)
...
COption_Skill::~COption_Skill(void)
{

}

HRESULT COption_Skill::Ready_Object(void)
{
	if(FAILED(Add_Component()))
		return E_FAIL;

	m_fX = (WINSIZEX >> 1) - 135.f;
	m_fY = (WINSIZEY >> 1);
...
HRESULT COption_Skill::Start_ForScene(void)
{
...
void COption_Skill::Update_Object(const _float& fTimeDelta)
{
...
void COption_Skill::Render_Object(void)
{
	...
	if(m_bClick || m_bCollision)
	{
		Engine::Render_Font(L"Font_Main_Data", L"(K) ½ºÅ³ ", &_vec2(m_fX - 30.f, m_fY - 45.f), D3DXCOLOR(1.f, 1.f, 1.f, m_fAlpha));
	}
}


HRESULT COption_Skill::Add_Component(void)
{
	...
	m_pNCTextureCom = (Engine::CTexture*)Engine::Clone_Resource(RESOURCE_STAGESTATIC, L"Option_Skill");
	...
	m_pCTextureCom = (Engine::CTexture*)Engine::Clone_Resource(RESOURCE_STAGESTATIC, L"Option_Skill_C");
	...
	m_pClickTextureCom = (Engine::CTexture*)Engine::Clone_Resource(RESOURCE_STAGESTATIC, L"Option_Skill_Click");
...
void COption_Skill::Alpha_Calculation(const _float& fTimeDelta)
{
	if(!m_bRender)
	{
...
void COption_Skill::PtInRect_Collision(const _float& fTimeDelta)
{
	POINT		ptMouse;
	...
	if(m_bCollision)
	{
		...
		else
		{
			if(m_bClick)
			{
				...
				CUI*	pUI	= ((CUI*)Engine::Find(L"Skill_Base"));
				if(pUI->GetRender())
					pUI->SetRender(false);
				...
				else
				{
					pUI->SetRender(true);
					::Safe_Release(pUI);
					pUI = ((CUI*)Engine::Find(L"Inven_Base"));
					if(pUI->GetRender())
						pUI->SetRender(false);
				}
				::Safe_Release(pUI);
...
void COption_Skill::Key_Check(void)
{
...
COption_Skill* COption_Skill::Create(LPDIRECT3DDEVICE9 pGraphicDev, const wstring& wstrName, const Engine::OBJECTSTATE& eState /*= Engine::STATE_DEFAULT*/, const _bool bIsActive /*= true*/)
{
	COption_Skill*		pInstance = new COption_Skill(pGraphicDev, wstrName, eState, bIsActive);
