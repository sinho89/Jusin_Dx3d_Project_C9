...
void CUpjuk_Center::Render_Object(void)
{
	...
	if (Engine::GetSceneID() == Engine::SCENE_STAGE_ICE)
		Engine::Render_Font(L"Font_Select_DunGeun", L"하누만 처치", &_vec2(m_fX - 40.f, m_fY - 10.f), D3DXCOLOR(1.f, 1.f, 1.f, m_fAlpha));
	else if (Engine::GetSceneID() == Engine::SCENE_STAGE_FIRE)
		Engine::Render_Font(L"Font_Select_DunGeun", L"바르디엘 처치", &_vec2(m_fX - 40.f, m_fY - 10.f), D3DXCOLOR(1.f, 1.f, 1.f, m_fAlpha));
}
