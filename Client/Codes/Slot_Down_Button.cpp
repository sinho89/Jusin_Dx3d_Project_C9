...
CSlot_Down_Button::CSlot_Down_Button(LPDIRECT3DDEVICE9 pGraphicDev, const wstring& wstrName, const Engine::OBJECTSTATE& eState /*= Engine::STATE_DEFAULT*/, const _bool bIsActive /*= true*/)
...
CSlot_Down_Button::~CSlot_Down_Button(void)
{

}

HRESULT CSlot_Down_Button::Ready_Object(void)
{
	if(FAILED(Add_Component()))
		return E_FAIL;

	m_fX = (WINSIZEX >> 1) + 270.f;
	m_fY = (WINSIZEY >> 1) + 420.f;
...
HRESULT CSlot_Down_Button::Start_ForScene(void)
{
...
void CSlot_Down_Button::Update_Object(const _float& fTimeDelta)
{
...
void CSlot_Down_Button::Render_Object(void)
{
...
HRESULT CSlot_Down_Button::Add_Component(void)
{
	...
	m_pNCTextureCom = (Engine::CTexture*)Engine::Clone_Resource(RESOURCE_STAGESTATIC, L"Main_Slot_Down_Button_NC");
	...
	m_pCTextureCom = (Engine::CTexture*)Engine::Clone_Resource(RESOURCE_STAGESTATIC, L"Main_Slot_Down_Button_C");
	...
	m_pClickTextureCom = (Engine::CTexture*)Engine::Clone_Resource(RESOURCE_STAGESTATIC, L"Main_Slot_Down_Button_Click");
...
void CSlot_Down_Button::PtInRect_Collision(const _float& fTimeDelta)
{
	POINT		ptMouse;
	...
	if(m_bCollision)
	{
		if(Engine::Get_DIMouseState(Engine::CInput_Device::DIM_LB) & 0x80)
		{
			...
			CUI* pUI = (CUI*)Engine::Find(L"Main_Base");
			if(pUI->GetSlotLevel() <= 1)
			{
				::Safe_Release(pUI);
				return;
			}
...
CSlot_Down_Button* CSlot_Down_Button::Create(LPDIRECT3DDEVICE9 pGraphicDev, const wstring& wstrName, const Engine::OBJECTSTATE& eState /*= Engine::STATE_DEFAULT*/, const _bool bIsActive /*= true*/)
{
	CSlot_Down_Button*		pInstance = new CSlot_Down_Button(pGraphicDev, wstrName, eState, bIsActive);
...
void CSlot_Down_Button::Free(void)
{
	CUI::Free();
}
