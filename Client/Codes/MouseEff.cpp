...
CMouseEff::CMouseEff(LPDIRECT3DDEVICE9 pGraphicDev, const wstring & wstrName)
	: CEffect(pGraphicDev, wstrName)
{
}

CMouseEff::~CMouseEff(void)
{
}

void CMouseEff::Update_Object(const _float & fTimeDelta)
{
	POINT		ptMouse;
	...
	m_fX = *((CUI*)m_pParent)->GetfX();
	m_fY = *((CUI*)m_pParent)->GetfY();
	...
	Update_RePart(); //렌더 설정
					 ...
					 //원하시는 렌더러 그룹에 넣어주세요 
	m_pRendererCom->Add_WindowUI(20000, this);

	D3DXMatrixOrthoLH(&m_matProj, float(WINSIZEX), float(WINSIZEY), 0.0f, 1.0f);
}

void CMouseEff::Render_Object(void)
{
...
HRESULT CMouseEff::Ready_Object(void)
{
	...
	m_iMaxFrame = 8;//스프라이트의 총개수
	m_iRow = 3;  //스프라이트 가로 개수
	m_iColumn = 3; //세로 개수
...
CMouseEff * CMouseEff::Create(LPDIRECT3DDEVICE9 pGraphicDev, const wstring & wstrName)
{
	CMouseEff*		pInstance = new CMouseEff(pGraphicDev, wstrName);
...
CMouseEff* CMouseEff::Create(LPDIRECT3DDEVICE9 pGraphicDev, const wstring& wstrName, Engine::CGameObject* pParentObject)
{
	CMouseEff*		pInstance = new CMouseEff(pGraphicDev, wstrName);
...
HRESULT CMouseEff::Add_Component(void)
{
	...
	m_pTextureCom = (Engine::CTexture*)Engine::Clone_Resource(RESOURCE_STATIC, L"CursorEff");
...
HRESULT CMouseEff::SetUp_ConstantTable(LPD3DXEFFECT pEffect)
{
	pEffect->AddRef();
...
void CMouseEff::Update_RePart(void)
{
	D3DXMATRIX  matX;
	D3DXMATRIX	matY;
	D3DXMATRIX	matZ;
...
void CMouseEff::SpriteUpdate(const _float & fTimeDelta)
{
	//스프라이트 업데이트를 돌려준다.
	Engine::TPART* pPrt = m_PrtD;
