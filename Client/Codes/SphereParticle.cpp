...
//=============Sphere Particle============//
//중점(Center Pos)을 기준으로 파티클들이 구의 형태로 퍼져있고
//중점을 향해서 이동한다.
//CenterPos = _vec3(0.0f, 0.0f, 0.0f)
//중점과의거리가 0에 가까워지면 새로 생성

CSphereParticle::CSphereParticle(LPDIRECT3DDEVICE9 pGraphicDev, const wstring& wstrName, _int Count)
:CEffect(pGraphicDev, wstrName)
, m_bRenderGroupCheck(false)
, m_fGoalResult(0.0f)
...
CSphereParticle::~CSphereParticle(void)
{
}

void CSphereParticle::Update_Object(const _float & fTimeDelta)
{
	if (!m_bAni)
		return;
...
void CSphereParticle::Render_Object(void)
{
	if (!m_bAni)
		return;
...
HRESULT CSphereParticle::Ready_Object(void)
{
...
CSphereParticle * CSphereParticle::Create(LPDIRECT3DDEVICE9 pGraphicDev, const wstring & wstrName, _int Count)
{
	CSphereParticle*		pInstance = new CSphereParticle(pGraphicDev, wstrName, Count);

	if (FAILED(pInstance->Ready_Object()))
	{
		MSG_BOX("SphereParticle Created Failed");
		::Safe_Release(pInstance);
	}
	return pInstance;
}

void CSphereParticle::Free(void)
{
	CEffect::Free();
	Engine::CEffect::Destroy();
}

HRESULT CSphereParticle::Add_Component(void)
{
...
HRESULT CSphereParticle::SetUp_ConstantTable(LPD3DXEFFECT pEffect)
{
	pEffect->AddRef();
...
void CSphereParticle::SetRenderGroupCheck(_bool bCheck)
{
	m_bRenderGroupCheck = bCheck;
}

void CSphereParticle::SetGoalResult(_float fResult)
{
	m_fGoalResult = fResult;
}

void CSphereParticle::SetPart(_int nIdx)
{
	...
	_float Radius = (_float)(rand() %15);



	pPrt->IntP.x = (_float)(Radius * sinf(fPhiX) * cosf(fPhiY));
	pPrt->IntP.y = (_float)(Radius * cosf(fPhiX));
	pPrt->IntP.z = (_float)(Radius * sinf(fPhiX) * sinf(fPhiY));

	//나중에 써먹자
	/*pPrt->IntP.x = Radius - 2.f  * cosf(fPhiX);
	pPrt->IntP.z = Radius - 2.f * sinf(fPhiX);
	pPrt->IntP.y = sqrtf(powf(Radius, 2.f) - powf(pPrt->IntP.x, 2) - powf(pPrt->IntP.z, 2));
	*/
	...
	_float fPScale = ((rand() % 40) + 2.f) * 0.004f;
...
void CSphereParticle::MoveUpdate(void)
{
...
void CSphereParticle::Update_RePart(void)
{
	D3DXMATRIX  matX;
	D3DXMATRIX	matY;
	D3DXMATRIX	matZ;
	...
	for (_int i = 0; i < m_pParticleInfo.m_PrtN; ++i)
	{
		...
		{
			TransmatWorld = TransmatWorld * (*m_matTargetWorld);
		...
		{
			if (m_bBillboardRotate)
				m_matWorld = matS  * matBill * TransmatWorld /** (*m_matTargetWorld)*/ * matT;
			else
...
void CSphereParticle::SetAni(_bool bAni)
{
	m_bAni = bAni;
...
//초기 설정
void CSphereParticle::InitSphereParticle(void)
{
	//scale
	m_pParticleInfo.m_vScale = _vec3(1.f, 1.0f, 1.0f);
...
void CSphereParticle::SetTargetWorld(_matrix * matTarget)
{
	m_matTargetWorld = matTarget;
}

void CSphereParticle::SetBillBoardRotateCheck(_bool bCheck)
{
	m_bBillboardRotate = bCheck;
}
