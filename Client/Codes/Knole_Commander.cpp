...
#include "AI.h"
#include "AI_Roaming_KCommander.h"
#include "AI_Discovery_KCommander.h"
#include "AI_Trace_KCommander.h"
#include "AI_Combat1_KCommander.h"
#include "AI_HandAttack1_KCommander.h"
#include "AI_BodySpin1_KCommander.h"
#include "AI_Combat2_KCommander.h"
#include "AI_SpinAttack2_KCommander.h"
#include "AI_Attack2_KCommander.h"
#include "AI_Defence2_KCommander.h"
#include "AI_Down_KCommander.h"
#include "AI_Idle_KCommander.h"
#include "AI_Death_KCommander.h"
...
CKnole_Commander::CKnole_Commander(LPDIRECT3DDEVICE9 pGraphicDev, const wstring& wstrName)
	: CMonster(pGraphicDev, wstrName)
	,m_pDust(NULL)
	,m_bWeapon(false)
{
	m_eMonsterID = MON_KNOLE_COMMANDER;
}
CKnole_Commander::~CKnole_Commander(void)
{

}

HRESULT CKnole_Commander::Start_ForScene(void)
{
	CMonster::Start_ForScene();

	m_pSoundCom->Set_Sound(L"Discovery", L"Knole_Commander_Discovery.wav");
	m_pSoundCom->Set_Sound(L"Attack_1", L"Knole_Commander_Attack_1.wav");
	m_pSoundCom->Set_Sound(L"Attack_2", L"Knole_Commander_Attack_2.wav");
	m_pSoundCom->Set_Sound(L"Attack_3", L"Knole_Commander_Attack_3.wav");
	m_pSoundCom->Set_Sound(L"Attack_4", L"Knole_Commander_Attack_4.wav");

	m_pSoundCom->Set_Sound(L"Weapon_1", L"Knole_Commander_Weapon_1.wav");
	m_pSoundCom->Set_Sound(L"Weapon_2", L"Knole_Commander_Weapon_2.wav");
	m_pSoundCom->Set_Sound(L"Weapon_3", L"Knole_Commander_Weapon_3.wav");
	m_pSoundCom->Set_Sound(L"Weapon_4", L"Knole_Commander_Weapon_4.wav");

	m_pSoundCom->Set_Sound(L"Dam_1", L"Knole_Commander_Dam_1.wav");
	m_pSoundCom->Set_Sound(L"Dam_2", L"Knole_Commander_Dam_2.wav");
	m_pSoundCom->Set_Sound(L"Dam_3", L"Knole_Commander_Dam_3.wav");

	m_pSoundCom->Set_Sound(L"Death", L"Knole_Commander_Death.wav");

	m_pSoundCom->Set_Sound(L"Floor_Hit", L"Floor_Hit.wav");

	//// WorldUI
	Engine::CGameObject* pGameObject = CMonster_Name::Create(m_pGraphicDev, L"Monster_Name", this, MON_KNOLE_COMMANDER);
	if (pGameObject == NULL)
		return E_FAIL;
	Engine::AddObject(Engine::LAYER_UI, L"Name_UI", pGameObject);

	pGameObject = CWorld_HP_Gage::Create(m_pGraphicDev, L"HP_Gage", this, MON_KNOLE_COMMANDER);
	if (pGameObject == NULL)
		return E_FAIL;
	Engine::AddObject(Engine::LAYER_UI, L"Name_UI", pGameObject);

	pGameObject = CWorld_HP_Bar::Create(m_pGraphicDev, L"HP_Bar", this, MON_KNOLE_COMMANDER);
	if (pGameObject == NULL)
		return E_FAIL;
	Engine::AddObject(Engine::LAYER_UI, L"Name_UI", pGameObject);
	...
	pAI_State = CAI_Roaming_KCommander::Create(m_pTransCom, m_pAnimatorCom, m_pNaviCom, m_pAICom);
	pAI_State->Set_AniInfo(&m_vecAniInfo);
	if (pAI_State == NULL)
		return E_FAIL;
	m_pAICom->Set_State(_uint(AI_STATE::AI_ROAMING), pAI_State);

	pAI_State = CAI_Discovery_KCommander::Create(m_pTransCom, m_pAnimatorCom, m_pNaviCom, m_pCalculatorCom, m_pRigidbodyCom,
		m_pColliderCom, &m_tColInfo, &m_bDamaged, &m_bWeapon, m_pAICom, m_pSoundCom);
	pAI_State->Set_AniInfo(&m_vecAniInfo);
	if (pAI_State == NULL)
		return E_FAIL;
	m_pAICom->Set_State(_uint(AI_STATE::AI_DISCOVERY), pAI_State);

	pAI_State = CAI_Trace_KCommander::Create(m_pTransCom, m_pAnimatorCom, m_pNaviCom, m_pCalculatorCom, m_pRigidbodyCom,
		m_pColliderCom, &m_tColInfo, &m_bDamaged, &m_bWeapon, m_pAICom);
	pAI_State->Set_AniInfo(&m_vecAniInfo);
	if (pAI_State == NULL)
		return E_FAIL;
	m_pAICom->Set_State(_uint(AI_STATE::AI_TRACE), pAI_State);

	pAI_State = CAI_Combat1_KCommander::Create(m_pTransCom, m_pAnimatorCom, m_pNaviCom, m_pCalculatorCom, m_pRigidbodyCom,
		m_pColliderCom, &m_tColInfo, &m_bDamaged, &m_bWeapon, m_pAICom);
	pAI_State->Set_AniInfo(&m_vecAniInfo);
	if (pAI_State == NULL)
		return E_FAIL;
	m_pAICom->Set_State(_uint(AI_STATE::AI_COMBAT1), pAI_State);


	pAI_State = CAI_HandAttack1_KCommander::Create(m_pTransCom, m_pAnimatorCom, m_pNaviCom, m_pCalculatorCom, m_pRigidbodyCom,
		m_pColliderCom, &m_tColInfo, &m_bDamaged, &m_bWeapon, m_pAICom, m_pSoundCom);
	pAI_State->Set_AniInfo(&m_vecAniInfo);
	pAI_State->Set_SphereCol(m_pSphereColCom);
	if (pAI_State == NULL)
		return E_FAIL;
	m_pAICom->Set_State(_uint(AI_STATE::AI_HANDATTACK1), pAI_State);

	pAI_State = CAI_BodySpin1_KCommander::Create(m_pTransCom, m_pAnimatorCom, m_pNaviCom, m_pCalculatorCom, m_pRigidbodyCom,
		m_pColliderCom, &m_tColInfo, &m_bDamaged, &m_bWeapon, m_pAICom, m_pSoundCom);
	pAI_State->Set_AniInfo(&m_vecAniInfo);
	pAI_State->Set_SphereCol(m_pSphereColCom);
	if (pAI_State == NULL)
		return E_FAIL;
	m_pAICom->Set_State(_uint(AI_STATE::AI_BODYATTACK1), pAI_State);

	pAI_State = CAI_Combat2_KCommander::Create(m_pTransCom, m_pAnimatorCom, m_pNaviCom, m_pCalculatorCom, m_pRigidbodyCom,
		m_pColliderCom, &m_tColInfo, &m_bDamaged, &m_bWeapon, m_pAICom);
	pAI_State->Set_AniInfo(&m_vecAniInfo);
	pAI_State->Set_SphereCol(m_pSphereColCom);
	if (pAI_State == NULL)
		return E_FAIL;
	m_pAICom->Set_State(_uint(AI_STATE::AI_COMBAT2), pAI_State);

	pAI_State = CAI_SpinAttack2_KCommander::Create(m_pTransCom, m_pAnimatorCom, m_pNaviCom, m_pCalculatorCom, m_pRigidbodyCom,
		m_pColliderCom, &m_tColInfo, &m_bDamaged, &m_bWeapon, m_pAICom, m_pSoundCom);
	pAI_State->Set_AniInfo(&m_vecAniInfo);
	pAI_State->Set_SphereCol(m_pSphereColCom);
	if (pAI_State == NULL)
	...
	pAI_State = CAI_Attack2_KCommander::Create(m_pTransCom, m_pAnimatorCom, m_pNaviCom, m_pCalculatorCom, m_pRigidbodyCom,
		m_pColliderCom, &m_tColInfo, &m_bDamaged, &m_bWeapon, m_pAICom, m_pSoundCom);
	pAI_State->Set_AniInfo(&m_vecAniInfo);
	pAI_State->Set_SphereCol(m_pSphereColCom);
	if (pAI_State == NULL)
	...
	pAI_State = CAI_Defence2_KCommander::Create(m_pTransCom, m_pAnimatorCom, m_pNaviCom, m_pCalculatorCom, m_pRigidbodyCom,
		m_pColliderCom, &m_tColInfo, &m_bDamaged, &m_bWeapon, m_pAICom);
	pAI_State->Set_AniInfo(&m_vecAniInfo);
	pAI_State->Set_SphereCol(m_pSphereColCom);
	if (pAI_State == NULL)
	...
	pAI_State = CAI_Down_KCommander::Create(m_pTransCom, m_pAnimatorCom, m_pNaviCom, m_pCalculatorCom, m_pRigidbodyCom,
		m_pColliderCom, &m_tColInfo, &m_bDamaged, &m_bWeapon, m_pAICom);
	pAI_State->Set_AniInfo(&m_vecAniInfo);
	pAI_State->Set_SphereCol(m_pSphereColCom);
	if (pAI_State == NULL)
	...
	pAI_State = CAI_Idle_KCommander::Create(m_pTransCom, m_pAnimatorCom, m_pNaviCom, m_pCalculatorCom, m_pRigidbodyCom,
		m_pColliderCom, &m_tColInfo, &m_bDamaged, &m_bWeapon, m_pAICom);
	pAI_State->Set_AniInfo(&m_vecAniInfo);
	pAI_State->Set_SphereCol(m_pSphereColCom);
	if (pAI_State == NULL)
	...
	pAI_State = CAI_Death_KCommander::Create(m_pTransCom, m_pAnimatorCom, m_pNaviCom, m_pCalculatorCom, m_pRigidbodyCom,
		m_pColliderCom, &m_tColInfo, &m_bDamaged, &m_bWeapon, m_pAICom);
	pAI_State->Set_AniInfo(&m_vecAniInfo);
	pAI_State->Set_SphereCol(m_pSphereColCom);
	if (pAI_State == NULL)
...
void CKnole_Commander::Update_Object(const _float& fTimeDelta)
{
...
void CKnole_Commander::Render_Object(void)
{
...
void CKnole_Commander::Render_Object(LPD3DXEFFECT pEffect, const _uint iPassIdx)
{
	m_pMeshCom->Play_AnimationSet(0.f);
	...
	static_cast<Engine::CDynamicMesh*>(m_pMeshCom)->SetWeaponDrawCheck(m_bWeapon);
...
HRESULT CKnole_Commander::Ready_Object(void)
{
	if (FAILED(Add_Component()))
		return E_FAIL;

	if (FAILED(LoadAniInfo(L"Knole_Commander_Ani")))
		return E_FAIL;

	if (FAILED(LoadCollider(L"Knole_Commander_Collider")))
		return E_FAIL;
	...
	m_tMonInfo.iMaxHp = 300000;
	m_tMonInfo.iHp = m_tMonInfo.iMaxHp;
	m_tMonInfo.iAtt = 10;
	m_tMonInfo.m_eMonID = MON_KNOLE_COMMANDER;
	////애니메이션 상태 준비
	Engine::CAnimationState* pState = NULL;

	pState = CKnole_Commander_Ani::Create(m_pMeshCom, &m_pmapNextMotion[MONSTER_NORMAL], STAND_IDLE);
	if (pState == NULL)
		return E_FAIL;
	m_pAnimatorCom->Set_State(MONSTER_NORMAL, pState);

	pState = CKnole_Commander_Ani::Create(m_pMeshCom, &m_pmapNextMotion[MONSTER_FIGHT], DISCOVERY);
	if (pState == NULL)
		return E_FAIL;
...
HRESULT CKnole_Commander::Add_Component(void)
{
...
HRESULT CKnole_Commander::Set_ConstantTable(LPD3DXEFFECT pEffect)
{
	pEffect->AddRef();
...
HRESULT CKnole_Commander::Set_Shadow_ConstanTable(LPD3DXEFFECT pEffect)
{
	pEffect->AddRef();
...
HRESULT CKnole_Commander::Set_CACD_Shadow_ConstanTable(LPD3DXEFFECT pEffect)
{
	pEffect->AddRef();
...
void CKnole_Commander::Render_Ray(void)
{
	//_vec3			vPoint[2];
...
void CKnole_Commander::DamagedSound(void)
{
...
//충돌 이펙트 설정
void CKnole_Commander::InitEffectDust(void)
{
...
//충돌 이펙트 관리 함수
void CKnole_Commander::CollideEffectDust(const _float& fTimeDelta)
{
...
CKnole_Commander* CKnole_Commander::Create(LPDIRECT3DDEVICE9 pGraphicDev, const wstring& wstrName)
{
	CKnole_Commander*		pInstance = new CKnole_Commander(pGraphicDev, wstrName);

	if (FAILED(pInstance->Ready_Object()))
	{
		MSG_BOX("CKnole_Commander Created Failed");
		::Safe_Release(pInstance);
	}
	return pInstance;
}

void CKnole_Commander::Free(void)
{
...
HRESULT CKnole_Commander::LoadAniInfo(const _tchar* pFileName)
{
	if (pFileName == NULL)
		return E_FAIL;
