...
CAI_Death_BBow::CAI_Death_BBow(void)
	:CAI_Babegazi_Bow()
	, m_fDownTime(0.f)
	, m_bStand(false)
	, m_iDirection(0)
...
HRESULT CAI_Death_BBow::Ready_State(Engine::CTransform* pTransform, Engine::CAnimator* pAnimator,
...
void CAI_Death_BBow::Start_State(void)
{
...
void CAI_Death_BBow::Update_State(const _float & fTimeDelta)
{
...
void CAI_Death_BBow::End_State(void)
{
	ZeroMemory(m_bCol, sizeof(bool) * Engine::COL_INFO_END);
}

void CAI_Death_BBow::Collsion_Work(void)
{
	if (*m_pDamaged)
	{
...
CAI_Death_BBow * CAI_Death_BBow::Create(Engine::CTransform* pTransform, Engine::CAnimator* pAnimator,
	Engine::CNavigationMesh* pNaviMesh, Engine::CCalculator* pCalculator,
	Engine::CRigidbody* pRigidbody, Engine::CCollider* pCollider,
	Engine::COLINFO* pColInfo, _bool* pDamaged,
	Engine::CAI* pAI)
{
	CAI_Death_BBow* pInstance = new CAI_Death_BBow;

	if (FAILED(pInstance->Ready_State(pTransform, pAnimator, pNaviMesh, pCalculator, pRigidbody
	, pCollider, pColInfo, pDamaged, pAI)))
	{
		MSG_BOX("CAI_Death_BBow Created Failed");
