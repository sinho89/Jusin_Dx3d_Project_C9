...
CRiseAttack::CRiseAttack(LPDIRECT3DDEVICE9 pGraphicDev, const wstring& wstrName)
...
CRiseAttack::~CRiseAttack(void)
{

}

void CRiseAttack::Update_Object(const _float& fTimeDelta)
{
...
void CRiseAttack::Render_Object(void)
{
	if (m_pPlayerAction[CWitchBlade::ACTION_RISINGATTACK] == false) //Player가 기상공격 상태가 아니면 Render하지 않는다.
		return;
...
HRESULT CRiseAttack::Ready_Obejct(void)
{
	m_iCntX = 30;
	m_iCntZ = 2;
	m_iInterval = 1;
	...
	if(FAILED(Engine::Ready_Buffers(m_pGraphicDev, RESOURCE_STAGESTATIC, L"Buffer_RiseAttackTrail", Engine::BUFFER_TRAILTEX, m_iCntX, m_iCntZ, m_iInterval)))
	{
		MSG_BOX("Buffer_RiseAttackTrail Load Failed");
		return E_FAIL;
	}
...
CRiseAttack* CRiseAttack::Create(LPDIRECT3DDEVICE9 pGraphicDev , const wstring& wstrName)
{
	CRiseAttack*		pInstance = new CRiseAttack(pGraphicDev, wstrName);


	if(FAILED(pInstance->Ready_Obejct()))
	{
		MSG_BOX("RistAttack Created Failed");
...
void CRiseAttack::Free(void)
{
	Engine::CGameObject::Free();
	Engine::Safe_Delete_Array(m_pTrailVtx);
}

HRESULT CRiseAttack::Add_Component(void)
{
	...
	m_pBufferCom = (Engine::CTrail_Texture*)Engine::Clone_Resource(RESOURCE_STAGESTATIC, L"Buffer_RiseAttackTrail");
...
HRESULT CRiseAttack::SetUp_ConstantTable(LPD3DXEFFECT pEffect)
{
	pEffect->AddRef();
...
HRESULT CRiseAttack::Start_ForScene(void)
{
	if (m_bOnceStart)
		return S_OK;
	...
	m_pmatBoneDown = pMesh->Get_FrameByName(BoneDown);
	m_pmatBoneUp   = pMesh->Get_FrameByName(BoneUp);
