...
CAI_Down_KCommander::CAI_Down_KCommander(void)
	:CAI_Knole_Commander()
	, m_fDownTime(0.f)
	, m_bStand(false)
	, m_iDirection(0)
...
HRESULT CAI_Down_KCommander::Ready_State(Engine::CTransform * pTransform, Engine::CAnimator * pAnimator,
...
void CAI_Down_KCommander::Start_State(void)
{
...
void CAI_Down_KCommander::Update_State(const _float & fTimeDelta)
{
	...
	if (!m_bOne)
	{
		m_pAnimator->Set_AnimationSet(DOWN);
	...
	if (m_bStand)
	{
		m_pAnimator->Set_AnimationSet(STAND_UP);
		m_bStand = false;
		m_bDown = false;
	}
	if (m_bDown)
	...
	//Down.
	if (m_pAnimator->Is_AnimationEnd(STAND_UP, 0.1))
	{
		if (!*m_pWeapon)
		{
...
void CAI_Down_KCommander::End_State(void)
{
	ZeroMemory(m_bCol, sizeof(bool) * Engine::COL_INFO_END);
}

void CAI_Down_KCommander::Collsion_Work(void)
{
	if (*m_pDamaged)
	{
...
CAI_Down_KCommander * CAI_Down_KCommander::Create(Engine::CTransform * pTransform, Engine::CAnimator * pAnimator,
	Engine::CNavigationMesh* pNaviMesh, Engine::CCalculator* pCalculator, Engine::CRigidbody* pRigidbody
	, Engine::CCollider* pCollider, Engine::COLINFO* pColInfo, _bool* pDamaged, _bool* pWeapon
	, Engine::CAI* pAI)
{
	CAI_Down_KCommander* pInstance = new CAI_Down_KCommander;
	...
	{
		MSG_BOX("CAI_Down_KCommander Created Failed");
...
void CAI_Down_KCommander::Free(void)
{
