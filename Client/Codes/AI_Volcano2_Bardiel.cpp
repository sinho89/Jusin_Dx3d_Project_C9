...
CAI_Volcano2_Bardiel::CAI_Volcano2_Bardiel(void)
	:CAI_Bardiel()
	,m_bAttackCol1(false)
	,m_bAttackCol2(false)
	,m_bAttackCol3(false)
...
HRESULT CAI_Volcano2_Bardiel::Ready_State(Engine::CTransform * pTransform, Engine::CAnimator * pAnimator,
...
void CAI_Volcano2_Bardiel::Start_State(void)
{
...
void CAI_Volcano2_Bardiel::Update_State(const _float & fTimeDelta)
{
	...
	if (m_pAnimator->Get_AnimationIndex() == ATT_VOLCANO)
	{
		if (m_pAnimator->IsPlaying_ThisSection((*m_pvecAniInfo)[ATT_VOLCANO]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATT_VOLCANO]->tMotionInfo.dEndTime * 0.1,
			(*m_pvecAniInfo)[ATT_VOLCANO]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATT_VOLCANO]->tMotionInfo.dEndTime * 0.15))
		{
			m_pSound->MyPlaySound(L"Bardiel_Volcano_Voice");
		}
		if (m_pAnimator->IsPlaying_ThisSection((*m_pvecAniInfo)[ATT_VOLCANO]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATT_VOLCANO]->tMotionInfo.dEndTime * 0.1,
			(*m_pvecAniInfo)[ATT_VOLCANO]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATT_VOLCANO]->tMotionInfo.dEndTime * 0.15))
		{
		...
		if (m_pAnimator->IsPlaying_ThisSection((*m_pvecAniInfo)[ATT_VOLCANO]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATT_VOLCANO]->tMotionInfo.dEndTime * 0.5,
			(*m_pvecAniInfo)[ATT_VOLCANO]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATT_VOLCANO]->tMotionInfo.dEndTime * 0.55))
		{
			m_pSound->MyPlaySound(L"Bardiel_ChangePhase2_Weapon");
		}

		if (m_pAnimator->IsPlaying_ThisSection((*m_pvecAniInfo)[ATT_VOLCANO]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATT_VOLCANO]->tMotionInfo.dEndTime * 0.5,
			(*m_pvecAniInfo)[ATT_VOLCANO]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATT_VOLCANO]->tMotionInfo.dEndTime * 0.7))
		{
			if (!m_bAttackCol1)
			{
				m_bAttackCol1 = true;
				m_pSphereCollider->SetScale(25.f);
				m_pSphereCollider->SetPos(_vec3(0.f, 3.f, 50.f));
...
void CAI_Volcano2_Bardiel::End_State(void)
{
	ZeroMemory(m_bCol, sizeof(bool) * Engine::COL_INFO_END);
}

//void CAI_Volcano2_Bardiel::CubeRayCheck(void)
//{
...
void CAI_Volcano2_Bardiel::Collsion_Work(void)
{
	if (*m_pDamaged)
	{
...
CAI_Volcano2_Bardiel * CAI_Volcano2_Bardiel::Create(Engine::CTransform * pTransform, Engine::CAnimator * pAnimator,
	Engine::CNavigationMesh* pNaviMesh, Engine::CCalculator* pCalculator, Engine::CRigidbody* pRigidbody
	, Engine::CCollider* pCollider, Engine::COLINFO* pColInfo, _bool* pDamaged
	, Engine::CAI* pAI, CSound* pSound)
{
	CAI_Volcano2_Bardiel* pInstance = new CAI_Volcano2_Bardiel;
	...
	{
		MSG_BOX("CAI_Volcano2_Bardiel Created Failed");
...
void CAI_Volcano2_Bardiel::Free(void)
{
