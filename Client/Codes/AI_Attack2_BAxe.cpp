...
CAI_Attack2_BAxe::CAI_Attack2_BAxe(void)
	:CAI_Babegazi_Axe()
	, m_bAttackCol(false)
{
}
...
HRESULT CAI_Attack2_BAxe::Ready_State(Engine::CTransform * pTransform, Engine::CAnimator * pAnimator,
...
void CAI_Attack2_BAxe::Start_State(void)
{
	...
	//	m_pAnimator->Set_AnimationSet(JUMP_C);
	//}
//	m_pAnimator->Set_AnimationSet(ATTACK2);
}


void CAI_Attack2_BAxe::Update_State(const _float & fTimeDelta)
{
...
//////////////////어택시 위치값 이동..
	if (m_pAnimator->Get_AnimationIndex() == ATTACK2)
	{
		...
		if (m_pAnimator->IsPlaying_ThisSection((*m_pvecAniInfo)[ATTACK2]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATTACK2]->tMotionInfo.dEndTime * 0.3,
			(*m_pvecAniInfo)[ATTACK2]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATTACK2]->tMotionInfo.dEndTime * 0.4))
		{
			m_pSound->MyPlaySound(L"Attack_2");
		}
		if (m_pAnimator->IsPlaying_ThisSection((*m_pvecAniInfo)[ATTACK2]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATTACK2]->tMotionInfo.dEndTime * 0.4,
			(*m_pvecAniInfo)[ATTACK2]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATTACK2]->tMotionInfo.dEndTime * 0.5))
		{
			m_pSound->MyPlaySound(L"Weapon_2");
		}

		if (m_pAnimator->IsPlaying_ThisSection((*m_pvecAniInfo)[ATTACK2]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATTACK2]->tMotionInfo.dEndTime * 0.1,
			(*m_pvecAniInfo)[ATTACK2]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATTACK2]->tMotionInfo.dEndTime * 0.5))
		{
		...
		if (m_pAnimator->IsPlaying_ThisSection((*m_pvecAniInfo)[ATTACK2]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATTACK2]->tMotionInfo.dEndTime * 0.3,
			(*m_pvecAniInfo)[ATTACK2]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATTACK2]->tMotionInfo.dEndTime * 0.6))
		{
			if (!m_bAttackCol)
			{
				m_bAttackCol = true;
	...
	//Attack..
	if (m_pAnimator->Is_AnimationEnd(ATTACK2, 0.1))
	{
...
void CAI_Attack2_BAxe::End_State(void)
{
	ZeroMemory(m_bCol, sizeof(bool) * Engine::COL_INFO_END);
	m_bAttackCol = false;
}

void CAI_Attack2_BAxe::Collsion_Work(void)
{
	if (*m_pDamaged)
	{
...
CAI_Attack2_BAxe * CAI_Attack2_BAxe::Create(Engine::CTransform * pTransform, Engine::CAnimator * pAnimator,
	Engine::CNavigationMesh* pNaviMesh, Engine::CCalculator* pCalculator, Engine::CRigidbody* pRigidbody
	, Engine::CCollider* pCollider, Engine::COLINFO* pColInfo, _bool* pDamaged
	, Engine::CAI* pAI, CSound* pSound)
{
	CAI_Attack2_BAxe* pInstance = new CAI_Attack2_BAxe;
	...
	{
		MSG_BOX("CAI_Attack2_BAxe Created Failed");
...
void CAI_Attack2_BAxe::Free(void)
{
