...
CAI_Trace_BBow::CAI_Trace_BBow(void)
	:CAI_Babegazi_Bow()
	, m_fCompareTime(0.f)
	, m_bCompareDis(false)
	, m_bDamaged(false)
...
HRESULT CAI_Trace_BBow::Ready_State(Engine::CTransform * pTransform, Engine::CAnimator * pAnimator,
...
void CAI_Trace_BBow::Start_State(void)
{
...
void CAI_Trace_BBow::Update_State(const _float & fTimeDelta)
{
...
void CAI_Trace_BBow::End_State(void)
{
	ZeroMemory(m_bCol, sizeof(bool) * Engine::COL_INFO_END);
}

//void CAI_Trace_BBow::CubeRayCheck(void)
//{
...
void CAI_Trace_BBow::TraceMove(const _float& fTimeDelta)
{
...
_vec3 CAI_Trace_BBow::AStarMove(_vec3* pPos, const _float &fTimeDelta)
{
	if (m_BestList.empty())
	{
		return *pPos;
...
void CAI_Trace_BBow::Collsion_Work(void)
{
...
CAI_Trace_BBow * CAI_Trace_BBow::Create(Engine::CTransform * pTransform, Engine::CAnimator * pAnimator,
	Engine::CNavigationMesh* pNaviMesh, Engine::CCalculator* pCalculator, Engine::CRigidbody* pRigidbody
	, Engine::CCollider* pCollider, Engine::COLINFO* pColInfo, _bool* pDamaged
	, Engine::CAI* pAI)
{
	CAI_Trace_BBow* pInstance = new CAI_Trace_BBow;

	if (FAILED(pInstance->Ready_State(pTransform, pAnimator, pNaviMesh, pCalculator, pRigidbody, pCollider
		, pColInfo, pDamaged, pAI)))
	{
		MSG_BOX("CAI_Trace_BBow Created Failed");
