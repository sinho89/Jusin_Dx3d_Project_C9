...
CBuy_Base::CBuy_Base(LPDIRECT3DDEVICE9 pGraphicDev, const wstring& wstrName, const Engine::OBJECTSTATE& eState /*= Engine::STATE_DEFAULT*/, const _bool bIsActive /*= true*/)
...
CBuy_Base::~CBuy_Base(void)
{

}

HRESULT CBuy_Base::Ready_Object(void)
{
	...
	m_wSortNum = 20;
	m_bRender = false;
	m_eWinType = WINDOW_BUY;

	m_bIsStatic = true;
	return S_OK;
}

HRESULT CBuy_Base::Start_ForScene(void)
{
...
void CBuy_Base::Update_Object(const _float& fTimeDelta)
{
	if (m_bRender)
	{
...
void CBuy_Base::Render_Object(void)
{
	...
	if (m_pItem)
	{
		...
		case ITEM_S_RED_WEAPON:
			Engine::Render_Font(L"Font_Main_Button", L"헬 크로스", &_vec2(m_fX - 30.f, m_fY - 20.f), D3DXCOLOR(1.f, 0.3f, 0.f, 1.f));
			break;
	...
	Engine::Render_Font(L"Font_Main_Button", L"구입 하시겠습니까?", &_vec2(m_fX - 40.f, m_fY + 10.f), D3DXCOLOR(1.f, 1.f, 1.f, 1.f));


}


HRESULT CBuy_Base::Add_Component(void)
{
	...
	m_pTextureCom = (Engine::CTexture*)Engine::Clone_Resource(RESOURCE_STAGESTATIC, L"Buy_Base");
...
CBuy_Base* CBuy_Base::Create(LPDIRECT3DDEVICE9 pGraphicDev, const wstring& wstrName, const Engine::OBJECTSTATE& eState /*= Engine::STATE_DEFAULT*/, const _bool bIsActive /*= true*/)
{
	CBuy_Base*		pInstance = new CBuy_Base(pGraphicDev, wstrName, eState, bIsActive);
