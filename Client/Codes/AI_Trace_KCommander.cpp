...
CAI_Trace_KCommander::CAI_Trace_KCommander(void)
	:CAI_Knole_Commander()
	, m_fCompareTime(0.f)
	, m_bCompareDis(false)
	, m_dwIdx(0)
...
HRESULT CAI_Trace_KCommander::Ready_State(Engine::CTransform * pTransform, Engine::CAnimator * pAnimator,
...
void CAI_Trace_KCommander::Start_State(void)
{
...
void CAI_Trace_KCommander::Update_State(const _float & fTimeDelta)
{
	...
	if (m_bStop)
	{
		...
		{
			m_pAnimator->Set_AnimationSet(BATTLE_IDLE_2);
		}
...
void CAI_Trace_KCommander::End_State(void)
{
	ZeroMemory(m_bCol, sizeof(bool) * Engine::COL_INFO_END);
}

//void CAI_Trace_KCommander::CubeRayCheck(void)
//{
...
void CAI_Trace_KCommander::TraceMove(const _float& fTimeDelta)
{
	if (!m_bRun)
	{
		if (*m_pWeapon)		//무기찬상태
		{
			if (m_pAnimator->Get_AnimationIndex() == WEAPON_OUT)
			{
				if (m_pAnimator->IsPlaying_ThisSection((*m_pvecAniInfo)[WEAPON_OUT]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[WEAPON_OUT]->tMotionInfo.dEndTime * 0.4,
					(*m_pvecAniInfo)[WEAPON_OUT]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[WEAPON_OUT]->tMotionInfo.dEndTime * 0.6))
				{
					*m_pWeapon = false;
				}
				if (m_pAnimator->Set_AnimationSet(MOVE_RUN_READY))
				{
					m_bRun = true;
				}
			}
	...
	if (m_bRun)
	{
		if (m_bRayMove)
		{
			if (m_pAnimator->Get_AnimationIndex() == MOVE_RUN_READY)
			{
				if (m_pAnimator->IsPlaying_ThisSection((*m_pvecAniInfo)[MOVE_RUN_READY]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[MOVE_RUN_READY]->tMotionInfo.dEndTime * 0.4,
					(*m_pvecAniInfo)[MOVE_RUN_READY]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[MOVE_RUN_READY]->tMotionInfo.dEndTime * 0.9))
				{
			...
			if (m_pAnimator->Get_AnimationIndex() == MOVE_RUN)
			{
				if (m_pAnimator->IsPlaying_ThisSection((*m_pvecAniInfo)[MOVE_RUN]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[MOVE_RUN]->tMotionInfo.dEndTime * 0.1,
					(*m_pvecAniInfo)[MOVE_RUN]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[MOVE_RUN]->tMotionInfo.dEndTime * 0.9))
				{
					m_dwIdx = m_pNavigationMesh->Get_CurrentCellIndex(&m_vPos);
		...
		else // (m_bAStarMove)
		{
			...
			{
				if (m_pAnimator->IsPlaying_ThisSection((*m_pvecAniInfo)[MOVE_RUN]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[MOVE_RUN]->tMotionInfo.dEndTime * 0.1,
					(*m_pvecAniInfo)[MOVE_RUN]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[MOVE_RUN]->tMotionInfo.dEndTime * 0.7))
				{
...
_vec3 CAI_Trace_KCommander::AStarMove(_vec3* pPos, const _float &fTimeDelta)
{
	if (m_BestList.empty())
	{
		return *pPos;
...
void CAI_Trace_KCommander::Collsion_Work(void)
{
...
CAI_Trace_KCommander * CAI_Trace_KCommander::Create(Engine::CTransform * pTransform, Engine::CAnimator * pAnimator,
	Engine::CNavigationMesh* pNaviMesh, Engine::CCalculator* pCalculator, Engine::CRigidbody* pRigidbody
	, Engine::CCollider* pCollider, Engine::COLINFO* pColInfo, _bool* pDamaged ,_bool* pWeapon
	, Engine::CAI* pAI)
{
	CAI_Trace_KCommander* pInstance = new CAI_Trace_KCommander;
	...
	{
		MSG_BOX("CAI_Trace_KCommander Created Failed");
...
void CAI_Trace_KCommander::Free(void)
{
