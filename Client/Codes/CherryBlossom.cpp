...
CCherryBlossom::CCherryBlossom(LPDIRECT3DDEVICE9 pGraphicDev, const wstring & wstrName)
...
void CCherryBlossom::Update_Object(const _float & fTimeDelta)
{
	if (!m_bAni)
		return;
...
void CCherryBlossom::Render_Object(void)
{
	if (!m_bAni)
		return;
...
HRESULT CCherryBlossom::Ready_Object(void)
{
...
CCherryBlossom * CCherryBlossom::Create(LPDIRECT3DDEVICE9 pGraphicDev, const wstring & wstrName)
{
	CCherryBlossom*		pInstance = new CCherryBlossom(pGraphicDev, wstrName);

	if (FAILED(pInstance->Ready_Object()))
	{
		MSG_BOX("CherryBlossom Created Failed");
		::Safe_Release(pInstance);
	}
	return pInstance;
}

CCherryBlossom * CCherryBlossom::LoadCreate(LPDIRECT3DDEVICE9 pGraphicDev, const wstring & wstrName)
{
	CCherryBlossom*		pInstance = new CCherryBlossom(pGraphicDev, wstrName);
...
void CCherryBlossom::Free(void)
{
	CEffect::Free();
	Engine::CEffect::Destroy();
}

HRESULT CCherryBlossom::Add_Component(void)
{
...
HRESULT CCherryBlossom::SetUp_ConstantTable(LPD3DXEFFECT pEffect)
{
	pEffect->AddRef();
...
HRESULT CCherryBlossom::LoadReady_Object(void)
{
	if (FAILED(Add_Component()))
		return E_FAIL;
...
void CCherryBlossom::SetPart(_int nIdx)
{
	...
	if (m_bEqual)
	{
		pPrt->IntP.x = -(m_pParticleInfo.m_vParticleArrange.x / 2) + rand() % (int)m_pParticleInfo.m_vParticleArrange.x;
		pPrt->IntP.y = -(m_pParticleInfo.m_vParticleArrange.y / 2) + rand() % (int)m_pParticleInfo.m_vParticleArrange.y;
		pPrt->IntP.z = -(m_pParticleInfo.m_vParticleArrange.z / 2) + rand() % (int)m_pParticleInfo.m_vParticleArrange.z;
...
void CCherryBlossom::MoveUpdate(void)
{
...
void CCherryBlossom::Update_RePart(void)
{
	D3DXMATRIX  matX;
	D3DXMATRIX	matY;
	D3DXMATRIX	matZ;
...
void CCherryBlossom::SetAni(_bool bAni)
{
	m_bAni = bAni;
...
_bool CCherryBlossom::GetAni(void)
{
	return m_bAni;
}

void CCherryBlossom::SetEqual(_bool bEqual)
{
	m_bEqual = bEqual;
}
