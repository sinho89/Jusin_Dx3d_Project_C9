...
CAI_Defence_BWarrior::CAI_Defence_BWarrior(void)
...
HRESULT CAI_Defence_BWarrior::Ready_State(Engine::CTransform * pTransform, Engine::CAnimator * pAnimator,
...
void CAI_Defence_BWarrior::Start_State(void)
{
...
void CAI_Defence_BWarrior::Update_State(const _float & fTimeDelta)
{
	...
	if (m_pAnimator->Get_AnimationIndex() == DEFENCE_SWORD)
	{
		...
		m_bLookAtDir = false;
		if (m_pAnimator->IsPlaying_ThisSection((*m_pvecAniInfo)[ATTACK]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATTACK]->tMotionInfo.dEndTime * 0.0,
			(*m_pvecAniInfo)[ATTACK]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATTACK]->tMotionInfo.dEndTime * 0.1))
		{
			m_pSound->MyPlaySound(L"Defence_Sword");
		}
		if (m_pAnimator->IsPlaying_ThisSection((*m_pvecAniInfo)[ATTACK]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATTACK]->tMotionInfo.dEndTime * 0.2,
			(*m_pvecAniInfo)[ATTACK]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATTACK]->tMotionInfo.dEndTime * 0.3))
		{
...
void CAI_Defence_BWarrior::End_State(void)
{
	ZeroMemory(m_bCol, sizeof(bool) * Engine::COL_INFO_END);
	m_bAttackCol = false;
}

void CAI_Defence_BWarrior::Collsion_Work(void)
{
	if (*m_pDamaged)
	{
...
CAI_Defence_BWarrior *CAI_Defence_BWarrior::Create(Engine::CTransform * pTransform, Engine::CAnimator * pAnimator,
	Engine::CNavigationMesh* pNaviMesh, Engine::CCalculator* pCalculator, Engine::CRigidbody* pRigidbody
	, Engine::CCollider* pCollider, Engine::COLINFO* pColInfo, _bool* pDamaged
	, Engine::CAI* pAI, CSound* pSound)
{
	CAI_Defence_BWarrior* pInstance = new CAI_Defence_BWarrior;
	...
	{
		MSG_BOX("CAI_Defence_BWarrior Created Failed");
...
void CAI_Defence_BWarrior::Free(void)
{
