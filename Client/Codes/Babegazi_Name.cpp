#include "stdafx.h"
#include "Babegazi_Name.h"
#include "Export_Engine.h"
#include "PlayerCamera.h"
#include "Player.h"

CBabegazi_Name::CBabegazi_Name(LPDIRECT3DDEVICE9 pGraphicDev, const wstring & wstrName)
	: CEffect(pGraphicDev, wstrName)
{
}

CBabegazi_Name::~CBabegazi_Name(void)
{
}

void CBabegazi_Name::Update_Object(const _float & fTimeDelta)
{
...
void CBabegazi_Name::Render_Object(void)
{
...
HRESULT CBabegazi_Name::Ready_Object(void)
{
	...
	m_iMaxFrame = 1;//스프라이트의 총개수
	m_iRow = 1;  //스프라이트 가로 개수
	m_iColumn = 1; //세로 개수
	m_fSpriteSpeed = 0.f;//스프라이트 속도
...
CBabegazi_Name * CBabegazi_Name::Create(LPDIRECT3DDEVICE9 pGraphicDev, const wstring & wstrName)
{
	CBabegazi_Name*		pInstance = new CBabegazi_Name(pGraphicDev, wstrName);

	if (FAILED(pInstance->Ready_Object()))
	{
		MSG_BOX("CWorld_HP_Bar Created Failed");
...
CBabegazi_Name* CBabegazi_Name::Create(LPDIRECT3DDEVICE9 pGraphicDev, const wstring& wstrName, Engine::CGameObject* pParentObject)
{
	CBabegazi_Name*		pInstance = new CBabegazi_Name(pGraphicDev, wstrName);
...
HRESULT CBabegazi_Name::Add_Component(void)
{
	...
	m_pTextureCom = (Engine::CTexture*)Engine::Clone_Resource(RESOURCE_STAGESTATIC, L"Monster_Name");
...
HRESULT CBabegazi_Name::SetUp_ConstantTable(LPD3DXEFFECT pEffect)
{
	pEffect->AddRef();
...
void CBabegazi_Name::Update_RePart(void)
{
	D3DXMATRIX  matX;
	D3DXMATRIX	matY;
	D3DXMATRIX	matZ;
...
void CBabegazi_Name::SpriteUpdate(const _float & fTimeDelta)
{
	//스프라이트 업데이트를 돌려준다.
	Engine::TPART* pPrt = m_PrtD;
...
void CBabegazi_Name::SetPart(void)
{
	Engine::TPART* pPrt = m_PrtD;
