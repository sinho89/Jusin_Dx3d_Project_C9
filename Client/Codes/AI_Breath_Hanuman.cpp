...
CAI_Breath_Hanuman::CAI_Breath_Hanuman(void)
	:CAI_Hanuman()
	, m_fCompareTime(0.f)
	, m_bCompareDis(false)
	, m_bAttackCol1(false)
...
HRESULT CAI_Breath_Hanuman::Ready_State(Engine::CTransform * pTransform, Engine::CAnimator * pAnimator,
...
void CAI_Breath_Hanuman::Start_State(void)
{
...
void CAI_Breath_Hanuman::Update_State(const _float & fTimeDelta)
{
	...
	if (m_pAnimator->Get_AnimationIndex() == BREATH_0)
	{
	
		if (m_pAnimator->Set_AnimationSet(BREATH_1))
		{
	...
	if (m_pAnimator->Get_AnimationIndex() == BREATH_1)
	{
		if (m_pAnimator->IsPlaying_ThisSection((*m_pvecAniInfo)[BREATH_1]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[BREATH_1]->tMotionInfo.dEndTime * 0.2,
			(*m_pvecAniInfo)[BREATH_1]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[BREATH_1]->tMotionInfo.dEndTime * 0.3))
		{
			m_pSound->MyPlaySound(L"Breath_Voice");
		}
		if (m_pAnimator->IsPlaying_ThisSection((*m_pvecAniInfo)[BREATH_1]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[BREATH_1]->tMotionInfo.dEndTime * 0.2,
			(*m_pvecAniInfo)[BREATH_1]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[BREATH_1]->tMotionInfo.dEndTime * 0.8))
		{
			if (!m_bAttackCol1)
			{
				m_bAttackCol1 = true;
				m_pSphereCollider->SetScale(15.f);
				m_pSphereCollider->SetPos(_vec3(5.f, 3.f, 15.f));
...
void CAI_Breath_Hanuman::End_State(void)
{
	ZeroMemory(m_bCol, sizeof(bool) * Engine::COL_INFO_END);
}

void CAI_Breath_Hanuman::Collsion_Work(void)
{
	if (*m_pDamaged)
	{
...
CAI_Breath_Hanuman * CAI_Breath_Hanuman::Create(Engine::CTransform * pTransform, Engine::CAnimator * pAnimator,
	Engine::CNavigationMesh* pNaviMesh, Engine::CCalculator* pCalculator, Engine::CRigidbody* pRigidbody
	, Engine::CCollider* pCollider, Engine::COLINFO* pColInfo, _bool* pDamaged
	, Engine::CAI* pAI, CSound* pSound)
{
	CAI_Breath_Hanuman* pInstance = new CAI_Breath_Hanuman;
	...
	{
		MSG_BOX("CAI_Breath_Hanuman Created Failed");
...
void CAI_Breath_Hanuman::Free(void)
{
