...
#include "AI.h"
#include "AI_Roaming_BAxe.h"
#include "AI_Roaming_BAxe.h"
#include "AI_Trace_BAxe.h"
#include "AI_Discovery_BAxe.h"
#include "AI_Combat_BAxe.h"
#include "AI_Attack1_BAxe.h"
#include "AI_Attack2_BAxe.h"
#include "AI_Defence_BAxe.h"
#include "AI_Down_BAxe.h"
#include "AI_Idle_BAxe.h"
#include "AI_Death_BAxe.h"
...
CBabegazi_Axe::CBabegazi_Axe(LPDIRECT3DDEVICE9 pGraphicDev, const wstring& wstrName)
	: CMonster(pGraphicDev, wstrName)
	,m_pDust(NULL)
{
	m_eMonsterID = MON_B_Axe;
}
CBabegazi_Axe::~CBabegazi_Axe(void)
{

}

HRESULT CBabegazi_Axe::Start_ForScene(void)
{
	CMonster::Start_ForScene();

	m_pSoundCom->Set_Sound(L"Discovery", L"Babe_Axe_Discovery.wav");
	m_pSoundCom->Set_Sound(L"Attack_1", L"Babe_Axe_Attack_1.wav");
	m_pSoundCom->Set_Sound(L"Attack_2", L"Babe_Axe_Attack_2.wav");
	m_pSoundCom->Set_Sound(L"Defence_Sword", L"Babe_Axe_Def_Sword_1.wav");
	m_pSoundCom->Set_Sound(L"Weapon_1", L"Babe_Axe_Weapon_1.wav");
	m_pSoundCom->Set_Sound(L"Weapon_2", L"Babe_Axe_Weapon_2.wav");
	m_pSoundCom->Set_Sound(L"Weapon_3", L"Babe_Axe_Weapon_3.wav");
	m_pSoundCom->Set_Sound(L"Dam_1", L"Babe_Axe_Dam_1.wav");
	m_pSoundCom->Set_Sound(L"Dam_2", L"Babe_Axe_Dam_2.wav");
	m_pSoundCom->Set_Sound(L"Dam_3", L"Babe_Axe_Dam_3.wav");

	m_pSoundCom->Set_Sound(L"Death", L"Babe_Axe_Death.wav");
	m_pSoundCom->Set_Sound(L"Floor_Hit", L"Floor_Hit.wav");
	// WorldUI
	Engine::CGameObject* pGameObject = CMonster_Name::Create(m_pGraphicDev, L"Monster_Name", this, MON_B_Axe);
	if (pGameObject == NULL)
		return E_FAIL;
	Engine::AddObject(Engine::LAYER_UI, L"Name_UI", pGameObject);

	pGameObject = CWorld_HP_Gage::Create(m_pGraphicDev, L"HP_Gage", this, MON_B_Axe);
	if (pGameObject == NULL)
		return E_FAIL;
	Engine::AddObject(Engine::LAYER_UI, L"Name_UI", pGameObject);

	pGameObject = CWorld_HP_Bar::Create(m_pGraphicDev, L"HP_Bar", this, MON_B_Axe);
	if (pGameObject == NULL)
		return E_FAIL;
	Engine::AddObject(Engine::LAYER_UI, L"Name_UI", pGameObject);
	...
	pAI_State = CAI_Roaming_BAxe::Create(m_pTransCom, m_pAnimatorCom, m_pNaviCom, m_pAICom);
	pAI_State->Set_AniInfo(&m_vecAniInfo);
	if (pAI_State == NULL)
		return E_FAIL;
	m_pAICom->Set_State(_uint(AI_STATE::AI_ROAMING), pAI_State);

	pAI_State = CAI_Discovery_BAxe::Create(m_pTransCom, m_pAnimatorCom, m_pNaviCom, m_pCalculatorCom, m_pRigidbodyCom,
	...
	pAI_State = CAI_Trace_BAxe::Create(m_pTransCom, m_pAnimatorCom, m_pNaviCom, m_pCalculatorCom, m_pRigidbodyCom,
	...
	pAI_State = CAI_Combat_BAxe::Create(m_pTransCom, m_pAnimatorCom, m_pNaviCom, m_pCalculatorCom, m_pRigidbodyCom,
		m_pColliderCom, &m_tColInfo, &m_bDamaged, m_pAICom);
	pAI_State->Set_AniInfo(&m_vecAniInfo);
	if (pAI_State == NULL)
		return E_FAIL;
	...
	pAI_State = CAI_Attack1_BAxe::Create(m_pTransCom, m_pAnimatorCom, m_pNaviCom, m_pCalculatorCom, m_pRigidbodyCom,
		m_pColliderCom, &m_tColInfo, &m_bDamaged, m_pAICom, m_pSoundCom);
	pAI_State->Set_AniInfo(&m_vecAniInfo);
	...
	pAI_State = CAI_Attack2_BAxe::Create(m_pTransCom, m_pAnimatorCom, m_pNaviCom, m_pCalculatorCom, m_pRigidbodyCom,
		m_pColliderCom, &m_tColInfo, &m_bDamaged, m_pAICom, m_pSoundCom);
	pAI_State->Set_AniInfo(&m_vecAniInfo);
	pAI_State->Set_SphereCol(m_pSphereColCom);
	if (pAI_State == NULL)
	...
	pAI_State = CAI_Defence_BAxe::Create(m_pTransCom, m_pAnimatorCom, m_pNaviCom, m_pCalculatorCom, m_pRigidbodyCom,
		m_pColliderCom, &m_tColInfo, &m_bDamaged, m_pAICom, m_pSoundCom);
	pAI_State->Set_AniInfo(&m_vecAniInfo);
	pAI_State->Set_SphereCol(m_pSphereColCom);
	if (pAI_State == NULL)
	...
	pAI_State = CAI_Down_BAxe::Create(m_pTransCom, m_pAnimatorCom, m_pNaviCom, m_pCalculatorCom, m_pRigidbodyCom,
		m_pColliderCom, &m_tColInfo, &m_bDamaged, m_pAICom);
	pAI_State->Set_AniInfo(&m_vecAniInfo);
	if (pAI_State == NULL)
		return E_FAIL;
	...
	pAI_State = CAI_Idle_BAxe::Create(m_pTransCom, m_pAnimatorCom, m_pNaviCom, m_pCalculatorCom, m_pRigidbodyCom,
		m_pColliderCom, &m_tColInfo, &m_bDamaged, m_pAICom);
	pAI_State->Set_AniInfo(&m_vecAniInfo);
	if (pAI_State == NULL)
		return E_FAIL;
	...
	pAI_State = CAI_Death_BAxe::Create(m_pTransCom, m_pAnimatorCom, m_pNaviCom, m_pCalculatorCom, m_pRigidbodyCom,
		m_pColliderCom, &m_tColInfo, &m_bDamaged, m_pAICom);
	pAI_State->Set_AniInfo(&m_vecAniInfo);
	if (pAI_State == NULL)
		return E_FAIL;
...
void CBabegazi_Axe::Update_Object(const _float& fTimeDelta)
{
	if (!m_bDeath)
	{
		if (m_tMonInfo.iHp < 0)
...
void CBabegazi_Axe::Render_Object(void)
{
	if (m_pSphereColCom)
...
void CBabegazi_Axe::Render_Object(LPD3DXEFFECT pEffect, const _uint iPassIdx)
{
...
HRESULT CBabegazi_Axe::Ready_Object(void)
{
	if (FAILED(Add_Component()))
		return E_FAIL;

	if (FAILED(LoadAniInfo(L"Babegazi_Axe_Ani")))
		return E_FAIL;

	if (FAILED(LoadCollider(L"Babegazi_Axe_Collider")))
		return E_FAIL;
	...
	m_tMonInfo.iAtt = 10;
	m_fAlpha = 1.f;
	m_tMonInfo.m_eMonID = MON_B_Axe;

	////애니메이션 상태 준비
	Engine::CAnimationState* pState = NULL;

	pState = CBabegazi_Axe_Ani::Create(m_pMeshCom, &m_pmapNextMotion[MONSTER_NORMAL], CAPTURE);
	if (pState == NULL)
		return E_FAIL;
	m_pAnimatorCom->Set_State(MONSTER_NORMAL, pState);

	pState = CBabegazi_Axe_Ani::Create(m_pMeshCom, &m_pmapNextMotion[MONSTER_FIGHT], DISCOVERY);
	if (pState == NULL)
		return E_FAIL;
...
HRESULT CBabegazi_Axe::Add_Component(void)
{
...
HRESULT CBabegazi_Axe::Set_ConstantTable(LPD3DXEFFECT pEffect)
{
	pEffect->AddRef();
...
HRESULT CBabegazi_Axe::Set_Shadow_ConstanTable(LPD3DXEFFECT pEffect)
{
	pEffect->AddRef();
...
HRESULT CBabegazi_Axe::Set_CACD_Shadow_ConstanTable(LPD3DXEFFECT pEffect)
{
	pEffect->AddRef();
...
void CBabegazi_Axe::Render_Ray(void)
{
	//_vec3			vPoint[2];
...
void CBabegazi_Axe::DamagedSound(void)
{
	...
	{
		if (m_pAnimatorCom->IsPlaying_ThisSection((m_vecAniInfo)[DAM_UPPER_C]->tMotionInfo.dStartTime + (m_vecAniInfo)[DAM_UPPER_C]->tMotionInfo.dEndTime * 0.1,
			(m_vecAniInfo)[DAM_UPPER_C]->tMotionInfo.dStartTime + (m_vecAniInfo)[DAM_UPPER_C]->tMotionInfo.dEndTime * 0.2))
		{
...
//충돌 이펙트 설정
void CBabegazi_Axe::InitEffectDust(void)
{
...
//충돌 이펙트 관리 함수
void CBabegazi_Axe::CollideEffectDust(const _float& fTimeDelta)
{
...
CBabegazi_Axe* CBabegazi_Axe::Create(LPDIRECT3DDEVICE9 pGraphicDev, const wstring& wstrName)
{
	CBabegazi_Axe*		pInstance = new CBabegazi_Axe(pGraphicDev, wstrName);

	if (FAILED(pInstance->Ready_Object()))
	{
		MSG_BOX("CBabegazi_Axe Created Failed");
		::Safe_Release(pInstance);
	}
	return pInstance;
}

void CBabegazi_Axe::Free(void)
{
...
HRESULT CBabegazi_Axe::LoadAniInfo(const _tchar* pFileName)
{
	if (pFileName == NULL)
		return E_FAIL;
