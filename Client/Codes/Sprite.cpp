...
void CSprite::Render_Object(LPD3DXEFFECT pEffect, const _uint iPassIdx)
{
	if (!m_bAni)
		return;
...
void CSprite::Update_RePart(void)
{
	...
	for(_int i = 0; i < m_pParticleInfo.m_PrtN; ++i)
	{
		...
		if(m_pParticleInfo.m_bBillBoard)
		{
			if (m_bYRotCheck)
			{
				m_matWorld = matS  * matY *  matBill * matT;
			}
			else
			{
				if (m_bBillboardRotate)
...
void CSprite::SetRenderCheck(_bool bRenderCheck)
{
	m_bRenderCheck = bRenderCheck;
}

void CSprite::SetTargetWorld(_matrix * matTarget)
{
	m_matTargetWorld = matTarget;
}

void CSprite::SetBillboardRotate(_bool bBheck)
{
...
void CSprite::SetYRotCheck(_bool bCheck)
{
	m_bYRotCheck = bCheck;
}
