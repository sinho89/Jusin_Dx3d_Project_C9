...
CAI_SpinAttack2_KCommander::CAI_SpinAttack2_KCommander(void)
	:CAI_Knole_Commander()
	, m_bAttackCol(false)
	, m_bAttackTime(false)
	, m_fAttackTime(0.f)
...
HRESULT CAI_SpinAttack2_KCommander::Ready_State(Engine::CTransform * pTransform, Engine::CAnimator * pAnimator,
...
void CAI_SpinAttack2_KCommander::Start_State(void)
{
...
void CAI_SpinAttack2_KCommander::Update_State(const _float & fTimeDelta)
{
...
//////////////////어택시 위치값 이동..
	if (m_pAnimator->Get_AnimationIndex() == ATTACK_1)
	{
		m_dwIdx = m_pNavigationMesh->Get_CurrentCellIndex(&m_vPos);
		if (m_pAnimator->IsPlaying_ThisSection((*m_pvecAniInfo)[ATTACK_1]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATTACK_1]->tMotionInfo.dEndTime * 0.0,
			(*m_pvecAniInfo)[ATTACK_1]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATTACK_1]->tMotionInfo.dEndTime * 0.1))
		{
			m_pSound->MyPlaySound(L"Attack_3");
		}
		if (m_pAnimator->IsPlaying_ThisSection((*m_pvecAniInfo)[ATTACK_1]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATTACK_1]->tMotionInfo.dEndTime * 0.3,
			(*m_pvecAniInfo)[ATTACK_1]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATTACK_1]->tMotionInfo.dEndTime * 0.5))
		{
...
void CAI_SpinAttack2_KCommander::End_State(void)
{
	ZeroMemory(m_bCol, sizeof(bool) * Engine::COL_INFO_END);
	m_bAttackCol = false;
}

void CAI_SpinAttack2_KCommander::Collsion_Work(void)
{
	if (*m_pDamaged)
	{
...
CAI_SpinAttack2_KCommander * CAI_SpinAttack2_KCommander::Create(Engine::CTransform * pTransform, Engine::CAnimator * pAnimator,
	Engine::CNavigationMesh* pNaviMesh, Engine::CCalculator* pCalculator, Engine::CRigidbody* pRigidbody
	, Engine::CCollider* pCollider, Engine::COLINFO* pColInfo, _bool* pDamaged, _bool* pWeapon
	, Engine::CAI* pAI, CSound* pSound)
{
	CAI_SpinAttack2_KCommander* pInstance = new CAI_SpinAttack2_KCommander;
	...
	{
		MSG_BOX("CAI_SpinAttack2_KCommander Created Failed");
...
void CAI_SpinAttack2_KCommander::Free(void)
{
