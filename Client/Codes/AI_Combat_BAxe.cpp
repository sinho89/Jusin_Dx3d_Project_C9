...
CAI_Combat_BAxe::CAI_Combat_BAxe(void)
	:CAI_Babegazi_Axe()
	, m_fChangeTime(0.f)
	, m_bChange(false)
	, m_bLRMove(false)
...
HRESULT CAI_Combat_BAxe::Ready_State(Engine::CTransform * pTransform, Engine::CAnimator * pAnimator,
...
void CAI_Combat_BAxe::Start_State(void)
{
...
void CAI_Combat_BAxe::Update_State(const _float & fTimeDelta)
{
	...
	if (!m_bDamaged)
	{
		...
		if (m_bLRMove)
		{
			...
			if (m_fPlayerDis > 17.f || m_fPlayerDis < 7.f)
				m_bLRMove = false;
		}
	}
...
void CAI_Combat_BAxe::End_State(void)
{
	//찍기 와이값..돌리기
	_vec3 vAngle = m_pTransform->Get_Now_Angle();
	vAngle.x = 0.f;
...
void CAI_Combat_BAxe::Collsion_Work(void)
{
	if (*m_pDamaged)
	{
...
CAI_Combat_BAxe * CAI_Combat_BAxe::Create(Engine::CTransform * pTransform, Engine::CAnimator * pAnimator,
	Engine::CNavigationMesh* pNaviMesh, Engine::CCalculator* pCalculator, Engine::CRigidbody* pRigidbody
	, Engine::CCollider* pCollider, Engine::COLINFO* pColInfo, _bool* pDamaged
	, Engine::CAI* pAI)
{
	CAI_Combat_BAxe* pInstance = new CAI_Combat_BAxe;

	if (FAILED(pInstance->Ready_State(pTransform, pAnimator, pNaviMesh, pCalculator, pRigidbody, pCollider
		, pColInfo, pDamaged, pAI)))
	{
		MSG_BOX("CAI_Combat_BAxe Created Failed");
...
void CAI_Combat_BAxe::Free(void)
{
