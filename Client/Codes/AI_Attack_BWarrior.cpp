...
CAI_Attack_BWarrior::CAI_Attack_BWarrior(void)
...
HRESULT CAI_Attack_BWarrior::Ready_State(Engine::CTransform * pTransform, Engine::CAnimator * pAnimator,
...
void CAI_Attack_BWarrior::Start_State(void)
{
	...
	m_pAnimator->Set_AnimationSet(ATTACK);
}


void CAI_Attack_BWarrior::Update_State(const _float & fTimeDelta)
{
...
//////////////////어택시 위치값 이동..
	if (m_pAnimator->Get_AnimationIndex() == ATTACK)
	{
		m_dwIdx = m_pNavigationMesh->Get_CurrentCellIndex(&m_vPos);
		if (m_pAnimator->IsPlaying_ThisSection((*m_pvecAniInfo)[ATTACK]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATTACK]->tMotionInfo.dEndTime * 0.3,
			(*m_pvecAniInfo)[ATTACK]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATTACK]->tMotionInfo.dEndTime * 0.4))
		{
			m_pSound->MyPlaySound(L"Attack_1");
		}
		if (m_pAnimator->IsPlaying_ThisSection((*m_pvecAniInfo)[ATTACK]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATTACK]->tMotionInfo.dEndTime * 0.4,
			(*m_pvecAniInfo)[ATTACK]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATTACK]->tMotionInfo.dEndTime * 0.5))
		{
			m_pSound->MyPlaySound(L"Weapon_1");
		}
		if (m_pAnimator->IsPlaying_ThisSection((*m_pvecAniInfo)[ATTACK]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATTACK]->tMotionInfo.dEndTime * 0.4,
			(*m_pvecAniInfo)[ATTACK]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATTACK]->tMotionInfo.dEndTime * 0.7))
		{
			if (!m_bAttackCol)
			{
				m_bAttackCol = true;
	...
	//Attack..
	if (m_pAnimator->Is_AnimationEnd(ATTACK, 0.1))
	{
...
void CAI_Attack_BWarrior::End_State(void)
{
	ZeroMemory(m_bCol, sizeof(bool) * Engine::COL_INFO_END);
	m_bAttackCol = false;
}

void CAI_Attack_BWarrior::Collsion_Work(void)
{
	if (*m_pDamaged)
	{
...
CAI_Attack_BWarrior * CAI_Attack_BWarrior::Create(Engine::CTransform * pTransform, Engine::CAnimator * pAnimator,
	Engine::CNavigationMesh* pNaviMesh, Engine::CCalculator* pCalculator, Engine::CRigidbody* pRigidbody
	, Engine::CCollider* pCollider, Engine::COLINFO* pColInfo, _bool* pDamaged
	, Engine::CAI* pAI, CSound* pSound)
{
	CAI_Attack_BWarrior* pInstance = new CAI_Attack_BWarrior;
	...
	{
		MSG_BOX("CAI_Attack_BWarrior Created Failed");
...
void CAI_Attack_BWarrior::Free(void)
{
