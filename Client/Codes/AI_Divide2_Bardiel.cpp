...
CAI_Divide2_Bardiel::CAI_Divide2_Bardiel(void)
	:CAI_Bardiel()
	,m_bAttackCol1(false)
	,m_bAttackCol2(false)
	,m_bAttackCol3(false)
...
HRESULT CAI_Divide2_Bardiel::Ready_State(Engine::CTransform * pTransform, Engine::CAnimator * pAnimator,
...
void CAI_Divide2_Bardiel::Start_State(void)
{
...
void CAI_Divide2_Bardiel::Update_State(const _float & fTimeDelta)
{
	...
	if (m_pAnimator->Get_AnimationIndex() == ATT_DIVIDE)
	{
		if (m_pAnimator->IsPlaying_ThisSection((*m_pvecAniInfo)[ATT_DIVIDE]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATT_DIVIDE]->tMotionInfo.dEndTime * 0.0,
			(*m_pvecAniInfo)[ATT_DIVIDE]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATT_DIVIDE]->tMotionInfo.dEndTime * 0.1))
		{
			m_pSound->MyPlaySound(L"Bardiel_Divide_Voice");
		}
		if (m_pAnimator->IsPlaying_ThisSection((*m_pvecAniInfo)[ATT_DIVIDE]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATT_DIVIDE]->tMotionInfo.dEndTime * 0.2,
			(*m_pvecAniInfo)[ATT_DIVIDE]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATT_DIVIDE]->tMotionInfo.dEndTime * 0.3))
		{
			m_pSound->MyPlaySound(L"Bardiel_Swing_4");
		}
		if (m_pAnimator->IsPlaying_ThisSection((*m_pvecAniInfo)[ATT_DIVIDE]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATT_DIVIDE]->tMotionInfo.dEndTime * 0.35,
			(*m_pvecAniInfo)[ATT_DIVIDE]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATT_DIVIDE]->tMotionInfo.dEndTime * 0.4))
		{
			m_pSound->MyPlaySound(L"Bardiel_Swing_2");
		}
		if (m_pAnimator->IsPlaying_ThisSection((*m_pvecAniInfo)[ATT_DIVIDE]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATT_DIVIDE]->tMotionInfo.dEndTime * 0.45,
			(*m_pvecAniInfo)[ATT_DIVIDE]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATT_DIVIDE]->tMotionInfo.dEndTime * 0.5))
		{
			m_pSound->MyPlaySound(L"Bardiel_Swing_1");
		}
		if (m_pAnimator->IsPlaying_ThisSection((*m_pvecAniInfo)[ATT_DIVIDE]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATT_DIVIDE]->tMotionInfo.dEndTime * 0.35,
			(*m_pvecAniInfo)[ATT_DIVIDE]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATT_DIVIDE]->tMotionInfo.dEndTime * 0.45))
		{
			m_pSound->MyPlaySound(L"Bardiel_Swing_5");
			if (!m_bAttackCol1)
			{
				m_bAttackCol1 = true;
...
void CAI_Divide2_Bardiel::End_State(void)
{
	ZeroMemory(m_bCol, sizeof(bool) * Engine::COL_INFO_END);
}

//void CAI_Divide2_Bardiel::CubeRayCheck(void)
//{
...
void CAI_Divide2_Bardiel::Collsion_Work(void)
{
	if (*m_pDamaged)
	{
...
CAI_Divide2_Bardiel * CAI_Divide2_Bardiel::Create(Engine::CTransform * pTransform, Engine::CAnimator * pAnimator,
	Engine::CNavigationMesh* pNaviMesh, Engine::CCalculator* pCalculator, Engine::CRigidbody* pRigidbody
	, Engine::CCollider* pCollider, Engine::COLINFO* pColInfo, _bool* pDamaged
	, Engine::CAI* pAI, CSound* pSound)
{
	CAI_Divide2_Bardiel* pInstance = new CAI_Divide2_Bardiel;
	...
	{
		MSG_BOX("CAI_Divide2_Bardiel Created Failed");
...
void CAI_Divide2_Bardiel::Free(void)
{
