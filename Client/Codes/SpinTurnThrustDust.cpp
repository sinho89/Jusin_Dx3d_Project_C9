...
CSpinTurnThrustDust::CSpinTurnThrustDust(LPDIRECT3DDEVICE9 pGraphicDev, const wstring & wstrName)
...
CSpinTurnThrustDust::~CSpinTurnThrustDust(void)
{
}

void CSpinTurnThrustDust::Update_Object(const _float & fTimeDelta)
{
	if (!m_bAni)
		return;
...
void CSpinTurnThrustDust::Render_Object(void)
{
	if (!m_bAni)
		return;
...
void CSpinTurnThrustDust::Render_Object(LPD3DXEFFECT pEffect, const _uint iPassIdx)
{
	if (!m_bAni)
		return;
...
void CSpinTurnThrustDust::Free(void)
{
	CEffect::Free();
...
HRESULT CSpinTurnThrustDust::Add_Component(void)
{
...
HRESULT CSpinTurnThrustDust::SetUp_ConstantTable(LPD3DXEFFECT pEffect)
{
	pEffect->AddRef();
...
void CSpinTurnThrustDust::SetPart(_int nIdx)
{
	...
	fPhi = nIdx * (_float)(360/m_pParticleInfo.m_PrtN);
	...
	// 초기 속도
	pPrt->IntV.x = cosf(fPhi);
	pPrt->IntV.y = 0.f;
	pPrt->IntV.z = sinf(fPhi);
...
void CSpinTurnThrustDust::MoveUpdate(const _float & fTimeDelta)
{
	//1.운동 갱신
...
void CSpinTurnThrustDust::Update_RePart(void)
{
	D3DXMATRIX  matX;
	D3DXMATRIX	matY;
	D3DXMATRIX	matZ;
...
CSpinTurnThrustDust * CSpinTurnThrustDust::LoadCreate(LPDIRECT3DDEVICE9 pGraphicDev, const wstring & wstrName)
{
	CSpinTurnThrustDust*		pInstance = new CSpinTurnThrustDust(pGraphicDev, wstrName);

	if (FAILED(pInstance->LoadReady_Object()))
	{
		MSG_BOX("SpinTurnThrustDust Created Failed");
...
void CSpinTurnThrustDust::SetAni(_bool bAni)
{
	m_bAni = bAni;
...
_bool CSpinTurnThrustDust::GetAni(void)
{
	return m_bAni;
}

void CSpinTurnThrustDust::SetAlphaDecrement(_float fAlpha)
{
	m_fAlphaDecrement = fAlpha;
}

HRESULT CSpinTurnThrustDust::LoadReady_Object(void)
{
	...
	m_pParticleInfo.m_tSpriteInfo.m_fSpriteSpeed = 40.f;
