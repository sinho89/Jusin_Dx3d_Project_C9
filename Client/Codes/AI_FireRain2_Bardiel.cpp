...
CAI_FireRain2_Bardiel::CAI_FireRain2_Bardiel(void)
	:CAI_Bardiel()
	,m_bAttackCol1(false)
	,m_bAttackCol2(false)
	,m_bAttackCol3(false)
...
HRESULT CAI_FireRain2_Bardiel::Ready_State(Engine::CTransform * pTransform, Engine::CAnimator * pAnimator,
...
void CAI_FireRain2_Bardiel::Start_State(void)
{
...
void CAI_FireRain2_Bardiel::Update_State(const _float & fTimeDelta)
{
	...
	if (m_pAnimator->Get_AnimationIndex() == ATT_FIRE_RAIN)
	{
		if (m_pAnimator->IsPlaying_ThisSection((*m_pvecAniInfo)[ATT_FIRE_RAIN]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATT_FIRE_RAIN]->tMotionInfo.dEndTime * 0.35,
			(*m_pvecAniInfo)[ATT_FIRE_RAIN]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATT_FIRE_RAIN]->tMotionInfo.dEndTime * 0.375))
		{
		...
		if (m_pAnimator->IsPlaying_ThisSection((*m_pvecAniInfo)[ATT_FIRE_RAIN]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATT_FIRE_RAIN]->tMotionInfo.dEndTime * 0.45,
			(*m_pvecAniInfo)[ATT_FIRE_RAIN]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATT_FIRE_RAIN]->tMotionInfo.dEndTime * 0.425))
		{
		...
		if (m_pAnimator->IsPlaying_ThisSection((*m_pvecAniInfo)[ATT_FIRE_RAIN]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATT_FIRE_RAIN]->tMotionInfo.dEndTime * 0.425,
			(*m_pvecAniInfo)[ATT_FIRE_RAIN]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATT_FIRE_RAIN]->tMotionInfo.dEndTime * 0.45))
		{
			if (!m_bAttackCol3)
			{
				Engine::Set_Shake(Engine::CAMERA_PLAYER, _vec3(0.f, 1.f, 0.f), 0.9f, 4);
		...
		if (m_pAnimator->IsPlaying_ThisSection((*m_pvecAniInfo)[ATT_FIRE_RAIN]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATT_FIRE_RAIN]->tMotionInfo.dEndTime * 0.45,
			(*m_pvecAniInfo)[ATT_FIRE_RAIN]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATT_FIRE_RAIN]->tMotionInfo.dEndTime * 0.5))
		{
...
void CAI_FireRain2_Bardiel::End_State(void)
{
	ZeroMemory(m_bCol, sizeof(bool) * Engine::COL_INFO_END);
}

//void CAI_FireRain2_Bardiel::CubeRayCheck(void)
//{
...
void CAI_FireRain2_Bardiel::Collsion_Work(void)
{
	if (*m_pDamaged)
	{
...
CAI_FireRain2_Bardiel * CAI_FireRain2_Bardiel::Create(Engine::CTransform * pTransform, Engine::CAnimator * pAnimator,
	Engine::CNavigationMesh* pNaviMesh, Engine::CCalculator* pCalculator, Engine::CRigidbody* pRigidbody
	, Engine::CCollider* pCollider, Engine::COLINFO* pColInfo, _bool* pDamaged
	, Engine::CAI* pAI)
{
	CAI_FireRain2_Bardiel* pInstance = new CAI_FireRain2_Bardiel;

	if (FAILED(pInstance->Ready_State(pTransform, pAnimator, pNaviMesh, pCalculator, pRigidbody, pCollider
		, pColInfo, pDamaged, pAI)))
	{
		MSG_BOX("CAI_FireRain2_Bardiel Created Failed");
...
void CAI_FireRain2_Bardiel::Free(void)
{
