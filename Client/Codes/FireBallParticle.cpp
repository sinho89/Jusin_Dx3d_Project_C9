...
CFireBallParticle::CFireBallParticle(LPDIRECT3DDEVICE9 pGraphicDev, const wstring & wstrName, _int Count)
...
CFireBallParticle::~CFireBallParticle(void)
{
}

void CFireBallParticle::Update_Object(const _float & fTimeDelta)
{
	if (!m_bAni)
		return;
...
void CFireBallParticle::Render_Object(void)
{
	if (!m_bAni)
		return;
...
void CFireBallParticle::Render_Object(LPD3DXEFFECT pEffect, const _uint iPassIdx)
{
	if (!m_bAni)
		return;
...
HRESULT CFireBallParticle::Ready_Object(void)
{
...
CFireBallParticle * CFireBallParticle::Create(LPDIRECT3DDEVICE9 pGraphicDev, const wstring & wstrName, _int Count)
{
	CFireBallParticle*		pInstance = new CFireBallParticle(pGraphicDev, wstrName, Count);

	if (FAILED(pInstance->Ready_Object()))
	{
		MSG_BOX("FireBallParticle Created Failed");
		::Safe_Release(pInstance);
	}
	return pInstance;
}

void CFireBallParticle::Free(void)
{
	CEffect::Free();
	Engine::CEffect::Destroy();
}

HRESULT CFireBallParticle::Add_Component(void)
{
...
HRESULT CFireBallParticle::SetUp_ConstantTable(LPD3DXEFFECT pEffect)
{
	pEffect->AddRef();
...
void CFireBallParticle::SetRenderGroupCheck(_bool bCheck)
{
	m_bRenderGroupCheck = bCheck;
}

void CFireBallParticle::SetGoalResult(_float fResult)
{
	m_fGoalResult = fResult;
}

void CFireBallParticle::SetPart(_int nIdx)
{
...
void CFireBallParticle::MoveUpdate(void)
{
...
void CFireBallParticle::Update_RePart(void)
{
	D3DXMATRIX  matX;
	D3DXMATRIX	matY;
	D3DXMATRIX	matZ;
...
void CFireBallParticle::SetAni(_bool bAni)
{
	m_bAni = bAni;
...
void CFireBallParticle::InitSphereParticle(void)
{
	//scale
	m_pParticleInfo.m_vScale = _vec3(1.0f, 1.0f, 1.0f);
	...
	m_pParticleInfo.m_iTextureNumber = 19;
}
