...
CPartition_Button::CPartition_Button(LPDIRECT3DDEVICE9 pGraphicDev, const wstring& wstrName, const Engine::OBJECTSTATE& eState /*= Engine::STATE_DEFAULT*/, const _bool bIsActive /*= true*/)
...
CPartition_Button::~CPartition_Button(void)
{

}

HRESULT CPartition_Button::Ready_Object(void)
{
...
HRESULT CPartition_Button::Start_ForScene(void)
{
...
void CPartition_Button::Update_Object(const _float& fTimeDelta)
{
	...
	if (m_bRender)
	{
		if (m_bType)
		{
			m_fX = m_pParent->GetInfo().x - 55.f;
			m_fY = m_pParent->GetInfo().y + 75.f;
		}
		else
		{
			m_fX = m_pParent->GetInfo().x + 55.f;
...
void CPartition_Button::Render_Object(void)
{
...
HRESULT CPartition_Button::Add_Component(void)
{
...
void CPartition_Button::PtInRect_Collision(const _float& fTimeDelta)
{
	POINT		ptMouse;
	...
	if (m_bCollision)
	{
		if (Engine::Get_DIMouseState(Engine::CInput_Device::DIM_LB) & 0x80)
		{
			...
			{
				// 아이템 복사
				if (((CPartition_Base*)m_pParent)->GetItem() == NULL)
					return;
				else
				{
					((CPartition_Base*)m_pParent)->GetItem()->Get_Item_Info()->m_iCount -= *(m_pParent->GetPartitionMinCnt());
					...
					switch (((CPartition_Base*)m_pParent)->GetItem()->Get_Item_Info()->m_eItemType)
					{
					case ITEM_HPPOTION:
					...
					((CUI_Item*)pGameObject)->Get_Item_Info()->m_iCount = *(m_pParent->GetPartitionMinCnt());
					((CUI_Item*)pGameObject)->SetMousePick(true);

					m_pScene->FindLayer(Engine::LAYER_UI)->AddObject(L"UI_Window", pGameObject);

					CUI*	pCursor = ((CUI*)Engine::Find(L"Cursor"));

					((CCursor*)pCursor)->SetItem(((CUI_Item*)pGameObject));

					::Safe_Release(pSheet3);
					m_pParent->SetRender(false);
					((CPartition_Base*)m_pParent)->SetItem(NULL);
...
void CPartition_Button::SetParent(CUI* pParent)
{
	m_pParent = pParent;
}

CPartition_Button* CPartition_Button::Create(LPDIRECT3DDEVICE9 pGraphicDev, const wstring& wstrName, const Engine::OBJECTSTATE& eState /*= Engine::STATE_DEFAULT*/, const _bool bIsActive /*= true*/)
{
	CPartition_Button*		pInstance = new CPartition_Button(pGraphicDev, wstrName, eState, bIsActive);
...
CPartition_Button* CPartition_Button::Create(LPDIRECT3DDEVICE9 pGraphicDev, const wstring& wstrName, CUI* pParent, _bool bType, Engine::CScene* pScene, const Engine::OBJECTSTATE& eState /*= Engine::STATE_DEFAULT*/, const _bool bIsActive /*= true*/)
{
	CPartition_Button*		pInstance = new CPartition_Button(pGraphicDev, wstrName, eState, bIsActive);
...
void CPartition_Button::Free(void)
{
	CUI::Free();
}
