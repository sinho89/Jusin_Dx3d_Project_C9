...
CAI_Combat1_KCommander::CAI_Combat1_KCommander(void)
	:CAI_Knole_Commander()
	, m_fChangeTime(0.f)
	, m_bChange(false)
	, m_bLRMove(false)
...
HRESULT CAI_Combat1_KCommander::Ready_State(Engine::CTransform * pTransform, Engine::CAnimator * pAnimator,
...
void CAI_Combat1_KCommander::Start_State(void)
{
...
void CAI_Combat1_KCommander::Update_State(const _float & fTimeDelta)
{
	...
	if (!m_bDamaged)
	{
		if (m_bChange)
		{
			...
			if (i == 0)
			{
				if (m_pAnimator->Set_AnimationSet(HAND_START))
				{
					m_pAI->Set_State(AI_HANDATTACK1);
					return;
				}
			}
			else if (i == 1)
			{
				if (m_pAnimator->Set_AnimationSet(BODY_SPIN_A))
				{
					m_pAI->Set_State(AI_BODYATTACK1);
					return;
				}
			}
			else
			{
				if (m_pAnimator->Set_AnimationSet(MODE_2_BATTLE))
				{
		...
		if (m_bLRMove)
		{
			...
			if (m_iDirection == 0)
			{
					...
					(*m_pvecAniInfo)[WALK_L]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[WALK_L]->tMotionInfo.dEndTime * 0.9))
				{
					...
					m_pTransform->Set_Position(&m_pNavigationMesh->Move_OnNaviMesh(&m_vPos, &vRight, 8.f, m_dwIdx, fTimeDelta));
				}
			}
...
void CAI_Combat1_KCommander::End_State(void)
{
	//찍기 와이값..돌리기
	_vec3 vAngle = m_pTransform->Get_Now_Angle();
	vAngle.x = 0.f;
...
void CAI_Combat1_KCommander::Collsion_Work(void)
{
	if (*m_pDamaged)
	{
...
CAI_Combat1_KCommander * CAI_Combat1_KCommander::Create(Engine::CTransform * pTransform, Engine::CAnimator * pAnimator,
	Engine::CNavigationMesh* pNaviMesh, Engine::CCalculator* pCalculator, Engine::CRigidbody* pRigidbody
	, Engine::CCollider* pCollider, Engine::COLINFO* pColInfo, _bool* pDamaged, _bool* pWeapon
	, Engine::CAI* pAI)
{
	CAI_Combat1_KCommander* pInstance = new CAI_Combat1_KCommander;
	...
	{
		MSG_BOX("CAI_Combat1_KCommander Created Failed");
...
void CAI_Combat1_KCommander::Free(void)
{
