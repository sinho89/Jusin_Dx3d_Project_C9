...
CLaveTrail::CLaveTrail(LPDIRECT3DDEVICE9 pGraphicDev, const wstring & wstrName)
...
CLaveTrail::~CLaveTrail(void)
{
}

void CLaveTrail::Update_Object(const _float & fTimeDelta)
{
	if (!m_bAni)
		return;
...
void CLaveTrail::Render_Object(void)
{
	if (!m_bAni)
		return;
...
void CLaveTrail::Render_Object(LPD3DXEFFECT pEffect, const _uint iPassIdx)
{
	if (!m_bAni)
		return;
...
HRESULT CLaveTrail::Ready_Obejct(void)
{
	m_iCntX = 30;
	m_iCntZ = 2;
	m_iInterval = 1;

	if (FAILED(Engine::Ready_Buffers(m_pGraphicDev, RESOURCE_STAGESTATIC, L"Bardiel_Rave_RightLeg_Trail", Engine::BUFFER_TRAILTEX, m_iCntX, m_iCntZ, m_iInterval)))
	{
		MSG_BOX("Bardiel_Rave_RightLeg_Trail Load Failed");
		return E_FAIL;
	}
...
CLaveTrail * CLaveTrail::Create(LPDIRECT3DDEVICE9 pGraphicDev, const wstring & wstrName)
{
	CLaveTrail*		pInstance = new CLaveTrail(pGraphicDev, wstrName);

	if (FAILED(pInstance->Ready_Obejct()))
	{
		MSG_BOX("RaveTrail Created Failed");
...
void CLaveTrail::Free(void)
{
	Engine::Safe_Delete_Array(m_pTrailVtx);
	CEffect::Free();
}

HRESULT CLaveTrail::Add_Component(void)
{
	...
	m_pBufferCom = (Engine::CTrail_Texture*)Engine::Clone_Resource(RESOURCE_STAGESTATIC, L"Bardiel_Rave_RightLeg_Trail");
...
HRESULT CLaveTrail::SetUp_ConstantTable(LPD3DXEFFECT pEffect)
{
	pEffect->AddRef();
...
void CLaveTrail::SettingTrail(void)
{
	vStart = _vec3(0.0f, 0.0f, 0.0f);
	vEnd = _vec3(0.0f, 0.0f, 0.0f);
...
void CLaveTrail::SetTargetMatrix(const _matrix * pTarget_Start_Mat, const _matrix * pTarget_End_Mat)
{
	m_pTarget_Start_Mat = pTarget_Start_Mat;
	m_pTarget_End_Mat = pTarget_End_Mat;
}

void CLaveTrail::SetAni(_bool bAni)
{
	m_bAni = bAni;
}

_bool CLaveTrail::GetAni(void)
{
	return m_bAni;
}