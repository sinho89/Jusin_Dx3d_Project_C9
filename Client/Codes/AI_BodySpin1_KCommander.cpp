...
CAI_BodySpin1_KCommander::CAI_BodySpin1_KCommander(void)
	:CAI_Knole_Commander()
	, m_bAttackCol(false)
	, m_bAttackTime(false)
	, m_fAttackTime(0.f)
...
HRESULT CAI_BodySpin1_KCommander::Ready_State(Engine::CTransform * pTransform, Engine::CAnimator * pAnimator,
...
void CAI_BodySpin1_KCommander::Start_State(void)
{
...
void CAI_BodySpin1_KCommander::Update_State(const _float & fTimeDelta)
{
	...
	if (m_pAnimator->Get_AnimationIndex() == BODY_SPIN_A)
	{
		m_dwIdx = m_pNavigationMesh->Get_CurrentCellIndex(&m_vPos);
		if (m_pAnimator->IsPlaying_ThisSection((*m_pvecAniInfo)[BODY_SPIN_A]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[BODY_SPIN_A]->tMotionInfo.dEndTime * 0.0,
			(*m_pvecAniInfo)[BODY_SPIN_A]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[BODY_SPIN_A]->tMotionInfo.dEndTime * 0.1))
		{
			m_pSound->MyPlaySound(L"Attack_2");
		}
		if (m_pAnimator->IsPlaying_ThisSection((*m_pvecAniInfo)[BODY_SPIN_A]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[BODY_SPIN_A]->tMotionInfo.dEndTime * 0.5,
			(*m_pvecAniInfo)[BODY_SPIN_A]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[BODY_SPIN_A]->tMotionInfo.dEndTime * 0.8))
		{
			m_pSound->MyPlaySound(L"Weapon_2");
			if (!m_bAttackCol)
			{
				m_bAttackCol = true;
		...
		m_pAnimator->Set_AnimationSet(BODY_SPIN_B);
	}
	else
	{
		if (m_bAttackCol)
	...
	//////////////////어택시 위치값 이동..
	if (m_pAnimator->Get_AnimationIndex() == BODY_SPIN_B)
	{
		...
		m_pAnimator->Set_AnimationSet(BODY_SPIN_C);

	}
	///////////////////////////////////////////
	if (m_pAnimator->Get_AnimationIndex() == BODY_SPIN_C)
	{
		if (m_pAnimator->IsPlaying_ThisSection((*m_pvecAniInfo)[BODY_SPIN_C]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[BODY_SPIN_C]->tMotionInfo.dEndTime * 0.0,
			(*m_pvecAniInfo)[BODY_SPIN_C]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[BODY_SPIN_C]->tMotionInfo.dEndTime * 0.5))
		{
			_vec3 vEnd;
			vEnd = m_vPos + m_vLook * fTimeDelta * 15.f;
...
void CAI_BodySpin1_KCommander::End_State(void)
{
	ZeroMemory(m_bCol, sizeof(bool) * Engine::COL_INFO_END);
	m_bAttackCol = false;
}

void CAI_BodySpin1_KCommander::Collsion_Work(void)
{
	if (*m_pDamaged)
	{
...
CAI_BodySpin1_KCommander * CAI_BodySpin1_KCommander::Create(Engine::CTransform * pTransform, Engine::CAnimator * pAnimator,
	Engine::CNavigationMesh* pNaviMesh, Engine::CCalculator* pCalculator, Engine::CRigidbody* pRigidbody
	, Engine::CCollider* pCollider, Engine::COLINFO* pColInfo, _bool* pDamaged, _bool* pWeapon
	, Engine::CAI* pAI, CSound* pSound)
{
	CAI_BodySpin1_KCommander* pInstance = new CAI_BodySpin1_KCommander;
	...
	{
		MSG_BOX("CAI_BodySpin1_KCommander Created Failed");
...
void CAI_BodySpin1_KCommander::Free(void)
{
