...
CHanumanFire::CHanumanFire(LPDIRECT3DDEVICE9 pGraphicDev, const wstring & wstrName)
: CEffect(pGraphicDev, wstrName)
, m_bRenderCheck(TRUE)
, m_bBlurCheck(TRUE)
, m_pRightHandFire(NULL)
, m_pLeftHandFire(NULL)
, m_pRightShoulder(NULL)
, m_pLeftShoulder(NULL)
, m_pmatTarget(NULL)
, m_iPattern(0)
{
	m_bAni = false;
...
CHanumanFire::~CHanumanFire(void)
{
}

void CHanumanFire::Update_Object(const _float & fTimeDelta)
{
	if (g_bHanumanDead)
		return;
	...
	_vec3 vPos;
	memcpy(&vPos, &m_pmatTarget->m[3][0], sizeof(_vec3));

	if (vPos.x == 0.f && vPos.y == 0.f && vPos.z == 0.f)
	{
		return;
	}
	...
	{
	case 0://오른손
		memcpy(&vPoint, &m_pRightHandFire->m[3][0], sizeof(_vec3));
		vPoint.y += 2.f;
		D3DXVec3TransformCoord(&vPoint, &vPoint, m_pmatTarget);
		break;

	case 1: //왼손
		memcpy(&vPoint, &m_pLeftHandFire->m[3][0], sizeof(_vec3));
		vPoint.y += 2.f;
		D3DXVec3TransformCoord(&vPoint, &vPoint, m_pmatTarget);
		break;
	case 2: //오른쪽 어깨
		memcpy(&vPoint, &m_pRightShoulder->m[3][0], sizeof(_vec3));
		vPoint.y += 4.f;
		D3DXVec3TransformCoord(&vPoint, &vPoint, m_pmatTarget);
		break;

	case 3: //왼쪽 어깨
		memcpy(&vPoint, &m_pLeftShoulder->m[3][0], sizeof(_vec3));
		vPoint.y += 4.f;
		D3DXVec3TransformCoord(&vPoint, &vPoint, m_pmatTarget);
		break;
	}

	m_pTransCom->Set_Position(&vPoint);
	//===============================================================
...
void CHanumanFire::Render_Object(void)
{
	if (g_bHanumanDead)
		return;
...
void CHanumanFire::Render_Object(LPD3DXEFFECT pEffect, const _uint iPassIdx)
{
	if (g_bHanumanDead)
		return;
...
HRESULT CHanumanFire::Ready_Object(void)
{
...
CHanumanFire * CHanumanFire::Create(LPDIRECT3DDEVICE9 pGraphicDev, const wstring & wstrName)
{
	CHanumanFire*		pInstance = new CHanumanFire(pGraphicDev, wstrName);

	if (FAILED(pInstance->Ready_Object()))
	{
		MSG_BOX("HanumanFire Created Failed");
...
void CHanumanFire::Free(void)
{
	CEffect::Free();
	Engine::CEffect::Destroy();
}

HRESULT CHanumanFire::Add_Component(void)
{
...
HRESULT CHanumanFire::SetUp_ConstantTable(LPD3DXEFFECT pEffect)
{
	pEffect->AddRef();
...
void CHanumanFire::SetPart(_int nIdx)
{
	Engine::TPART* pPrt = &m_PrtD[nIdx];


	pPrt->IntP = _vec3(0.0f, 0.0f, 0.0f);
...
void CHanumanFire::MoveUpdate(const _float & fTimeDelta)
{
...
void CHanumanFire::Update_RePart(void)
{
	D3DXMATRIX  matX;
	D3DXMATRIX	matY;
	D3DXMATRIX	matZ;
...
void CHanumanFire::SetRenderCheck(_bool bRenderCheck)
{
	m_bRenderCheck = bRenderCheck;
}

void CHanumanFire::SetAni(_bool bAni)
{
	m_bAni = bAni;
...
_bool CHanumanFire::GetAni(void)
{
	return m_bAni;
}

CHanumanFire * CHanumanFire::LoadCreate(LPDIRECT3DDEVICE9 pGraphicDev, const wstring & wstrName)
{
	CHanumanFire*		pInstance = new CHanumanFire(pGraphicDev, wstrName);
...
HRESULT CHanumanFire::Start_ForScene(void)
{
	if (m_bOnceStart)
		return S_OK;
	...
	char* BoneRightHand = "_18";
	char* BoneLeftHand = "_36";
	char* BoneRightShoulder = "_16";
	char* BoneLeftShoulder = "_46";


	//오른쪽손
	m_pRightHandFire = pMesh->Get_FrameByName(BoneRightHand);
	//왼쪽손
	m_pLeftHandFire = pMesh->Get_FrameByName(BoneLeftHand);
	//오른쪽 어깨
	m_pRightShoulder = pMesh->Get_FrameByName(BoneRightShoulder);
	//왼쪽 어깨
	m_pLeftShoulder = pMesh->Get_FrameByName(BoneLeftShoulder);
...
void CHanumanFire::SetPattern(_int iPattern)
{
	m_iPattern = iPattern;
}

HRESULT CHanumanFire::LoadReady_Object(void)
{
	Start_ForScene();
