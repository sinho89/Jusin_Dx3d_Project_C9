...
CAI_Clean2_Bardiel::CAI_Clean2_Bardiel(void)
	:CAI_Bardiel()
	,m_bAttackCol1(false)
	,m_bAttackCol2(false)
	,m_bAttackCol3(false)
...
HRESULT CAI_Clean2_Bardiel::Ready_State(Engine::CTransform * pTransform, Engine::CAnimator * pAnimator,
...
void CAI_Clean2_Bardiel::Start_State(void)
{
...
void CAI_Clean2_Bardiel::Update_State(const _float & fTimeDelta)
{
	...
	if (m_pAnimator->Get_AnimationIndex() == ATT_CLEAN)
	{
		if (m_pAnimator->IsPlaying_ThisSection((*m_pvecAniInfo)[ATT_CLEAN]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATT_CLEAN]->tMotionInfo.dEndTime * 0.0,
			(*m_pvecAniInfo)[ATT_CLEAN]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATT_CLEAN]->tMotionInfo.dEndTime * 0.1))
		{
			m_pSound->MyPlaySound(L"Bardiel_Clean_Voice_1");
		}
		if (m_pAnimator->IsPlaying_ThisSection((*m_pvecAniInfo)[ATT_CLEAN]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATT_CLEAN]->tMotionInfo.dEndTime * 0.25,
			(*m_pvecAniInfo)[ATT_CLEAN]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATT_CLEAN]->tMotionInfo.dEndTime * 0.3))
		{
			...
			if (!m_bAttackCol1)
			{
				...
				m_pSphereCollider->SetPos(_vec3(0.f, 3.f, 20.f));
		...
		if (m_pAnimator->IsPlaying_ThisSection((*m_pvecAniInfo)[ATT_CLEAN]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATT_CLEAN]->tMotionInfo.dEndTime * 0.45,
			(*m_pvecAniInfo)[ATT_CLEAN]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATT_CLEAN]->tMotionInfo.dEndTime * 0.5))
		{
		...
		if (m_pAnimator->IsPlaying_ThisSection((*m_pvecAniInfo)[ATT_CLEAN]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATT_CLEAN]->tMotionInfo.dEndTime * 0.6,
			(*m_pvecAniInfo)[ATT_CLEAN]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATT_CLEAN]->tMotionInfo.dEndTime * 0.7))
		{
			m_pSound->MyPlaySound(L"Bardiel_Clean_Voice_2");
			m_vPos = m_vPos + m_vLook * fTimeDelta * m_fMonsterSpeed;
			m_pTransform->Set_Position(&m_vPos);
		}

		if (m_pAnimator->IsPlaying_ThisSection((*m_pvecAniInfo)[ATT_CLEAN]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATT_CLEAN]->tMotionInfo.dEndTime * 0.7,
			(*m_pvecAniInfo)[ATT_CLEAN]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATT_CLEAN]->tMotionInfo.dEndTime * 0.8))
		{
			...
			if (!m_bAttackCol3)
			{
				...
				m_bAttackCol3 = true;
				m_pSphereCollider->SetScale(20.f);
				m_pSphereCollider->SetPos(_vec3(-5.f, 3.f, 35.f));
				Engine::COLINFO tColInfo;
		...
		//if (m_pAnimator->IsPlaying_ThisSection((*m_pvecAniInfo)[ATT_CLEAN]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATT_CLEAN]->tMotionInfo.dEndTime * 0.5,
		//	(*m_pvecAniInfo)[ATT_CLEAN]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATT_CLEAN]->tMotionInfo.dEndTime * 0.6))
		//{
		//	//m_vPos = m_vPos + m_vLook * fTimeDelta * m_fMonsterSpeed;
		//	//m_pTransform->Set_Position(&m_vPos);
		//	if (!m_bAttackCol4)
		...
		//if (m_pAnimator->IsPlaying_ThisSection((*m_pvecAniInfo)[ATT_CLEAN]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATT_CLEAN]->tMotionInfo.dEndTime * 0.62,
		//	(*m_pvecAniInfo)[ATT_CLEAN]->tMotionInfo.dStartTime + (*m_pvecAniInfo)[ATT_CLEAN]->tMotionInfo.dEndTime * 0.7))
		//{
		//	//m_vPos = m_vPos + m_vLook * fTimeDelta * m_fMonsterSpeed;
		//	//m_pTransform->Set_Position(&m_vPos);
...
void CAI_Clean2_Bardiel::End_State(void)
{
	ZeroMemory(m_bCol, sizeof(bool) * Engine::COL_INFO_END);
}

//void CAI_Clean2_Bardiel::CubeRayCheck(void)
//{
...
void CAI_Clean2_Bardiel::Collsion_Work(void)
{
	if (*m_pDamaged)
	{
...
CAI_Clean2_Bardiel * CAI_Clean2_Bardiel::Create(Engine::CTransform * pTransform, Engine::CAnimator * pAnimator,
	Engine::CNavigationMesh* pNaviMesh, Engine::CCalculator* pCalculator, Engine::CRigidbody* pRigidbody
	, Engine::CCollider* pCollider, Engine::COLINFO* pColInfo, _bool* pDamaged
	, Engine::CAI* pAI, CSound* pSound)
{
	CAI_Clean2_Bardiel* pInstance = new CAI_Clean2_Bardiel;
	...
	{
		MSG_BOX("CAI_Clean2_Bardiel Created Failed");
...
void CAI_Clean2_Bardiel::Free(void)
{
