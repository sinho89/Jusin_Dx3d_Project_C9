...
CAI_Death_BAxe::CAI_Death_BAxe(void)
	:CAI_Babegazi_Axe()
	, m_fDownTime(0.f)
	, m_bStand(false)
	, m_iDirection(0)
...
HRESULT CAI_Death_BAxe::Ready_State(Engine::CTransform* pTransform, Engine::CAnimator* pAnimator,
...
void CAI_Death_BAxe::Start_State(void)
{
...
void CAI_Death_BAxe::Update_State(const _float & fTimeDelta)
{
...
void CAI_Death_BAxe::End_State(void)
{
	ZeroMemory(m_bCol, sizeof(bool) * Engine::COL_INFO_END);
}

void CAI_Death_BAxe::Collsion_Work(void)
{
	if (*m_pDamaged)
	{
...
CAI_Death_BAxe * CAI_Death_BAxe::Create(Engine::CTransform* pTransform, Engine::CAnimator* pAnimator,
	Engine::CNavigationMesh* pNaviMesh, Engine::CCalculator* pCalculator,
	Engine::CRigidbody* pRigidbody, Engine::CCollider* pCollider,
	Engine::COLINFO* pColInfo, _bool* pDamaged,
	Engine::CAI* pAI)
{
	CAI_Death_BAxe* pInstance = new CAI_Death_BAxe;

	if (FAILED(pInstance->Ready_State(pTransform, pAnimator, pNaviMesh, pCalculator, pRigidbody
	, pCollider, pColInfo, pDamaged, pAI)))
	{
		MSG_BOX("CAI_Death_BAxe Created Failed");
